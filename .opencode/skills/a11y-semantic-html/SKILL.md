...
## このスキルを使う条件
- 既存UIの実装/改修で、`div`/`span` クリック要素、ラベル欠落、見出し構造崩れ、ARIA不整合を解消したい
- デザインシステムや共通コンポーネントで、Name/Role/Valueを再発しにくい形で担保したい

## このスキルを使わない条件
- 主課題が色コントラスト、フォーカス可視化、ターゲットサイズ、時間制限などで、セマンティクス/ARIAが主因でない
- 実装変更を伴わない純粋な文書整備のみを行う

## 出力契約
- 変更した要素/属性（例: `button` 化、`label` 紐付け、`aria-*` 同期）をファイル単位で示す
- どのWCAG達成基準（特に 1.3.1 / 2.4.6 / 4.1.2）を満たすための修正かを対応付ける
- 未対応の制約やフォローアップがあれば、理由と次アクションを明示する
