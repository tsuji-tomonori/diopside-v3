...
## このスキルを使う条件
- 既存画面のレスポンシブ改修やアクセシビリティ改善で、320px幅/200%ズーム時の破綻を防ぎたい
- 固定ヘッダ・サイドバー・表・カードグリッドなど、狭幅時に崩れやすいレイアウトを扱う
- ホバーで補足情報や操作を表示するUI（ツールチップ、メニュー、補助アクション）がある

## このスキルを使わない条件
- 動画字幕・代替テキスト・フォームエラー表示など、主課題がズーム/リフロー以外の達成基準である
- 画面改修ではなく、達成基準のスコープ策定や監査計画のみを作成する

## 出力契約
- 対象画面ごとに、1.4.10/1.4.4/1.3.4/1.4.12/1.4.13（必要に応じて2.4.11）の対応方針を示す
- 320px幅と200%ズームを前提に、横スクロール・重なり・情報欠落の有無を判断できる受入基準を示す
- ホバー/フォーカス出現UIが Dismissible/Hoverable/Persistent を満たす実装方針または修正観点を示す
