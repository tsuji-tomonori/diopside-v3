...
## このスキルを使う条件
- ドラッグ&ドロップ、スライダー、地図操作など、ドラッグ依存のUIを実装・改修するとき
- 24px未満になりやすい小型ターゲット（アイコンボタン、行内リンク、密集チップ）を含む画面を扱うとき
- モバイル操作時の誤タップや、破壊的操作の誤起動リスクを下げたいとき

## このスキルを使わない条件
- ポインター操作UIの新規追加/変更がなく、文言・翻訳・配色のみを変更するとき
- 主課題が動画字幕、コントラスト、認証、キーボードフォーカスなど別領域で、2.5.xの論点が中心でないとき

## 出力契約
- 対象画面ごとに、2.5.7/2.5.8を中心とした適合方針（どこで24pxを確保し、どこで例外を使うか）を示す
- ドラッグ必須操作ごとに、同等結果を達成できる非ドラッグ代替手段を示す
- 重要/破壊的操作の誤起動防止（キャンセル、確認、Undoなど）と最小手動テスト観点を示す
