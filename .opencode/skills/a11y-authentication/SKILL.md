...
## このスキルを使う条件
- ログイン/再認証/MFA/CAPTCHAを含む認証フローを新規実装または改修する
- 認証で「記憶・解読・転記」を要求していないか、3.3.8観点でレビューしたい
- セキュリティ要件を維持したまま、パスワードマネージャやペースト許可を設計に落とし込みたい

## このスキルを使わない条件
- 認証を含まない一般フォームのラベル/エラー改善が主目的（`a11y-forms-errors`を優先）
- 主課題がセッション延長通知やタイムアウト救済の設計（`a11y-time-limits`を優先）
- 主課題が認証外UIのフォーカス遷移やキーボード操作のみ（`a11y-keyboard-focus`を優先）

## 出力契約（このスキルの成果物）
- 認証フローごとの3.3.8適合判定（該当SC、現状、違反/リスク、根拠）
- ログイン/MFA/CAPTCHAに対する実装差分案（代替手段、入力補助、回復導線を含む）
- 検証可能な受入基準と最小手動テスト手順（キーボード、支援技術、コピペ/自動入力）
