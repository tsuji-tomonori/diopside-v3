...
13. FRカテゴリ正本（ADM/SCH/TAG/LST/DET/HLW/WCL）を維持し、実装工程単位カテゴリへの回帰を禁止する。
14. 管理者操作FR（収集起動/監視/再収集/公開運用/配信経路確認）を `01.管理画面(ADM)` へ集約する規則を関連スキルへ同期する。
15. 生成系FRは独立カテゴリ化せず、利用者機能カテゴリに配置する規則を関連スキルへ同期する。
16. RQ文書の `## 変更履歴` 各行に、関連RDRリンク（`[[RQ-RDR-xxx]]`）を必須化する規約を関連スキルへ同期する。
