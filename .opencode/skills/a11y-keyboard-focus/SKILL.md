...
## このスキルを使う条件
- Tab移動、フォーカス順序、フォーカス可視化、モーダル時のフォーカス制御に不具合がある
- 固定ヘッダ/トースト/オーバーレイの重なりで、フォーカス対象が隠れる懸念がある
- 新規画面や主要導線の受入前に、WCAG 2.2のキーボード/フォーカス要件をまとめて確認したい

## このスキルを使わない条件
- 色コントラストや代替テキストのみの監査など、キーボード/フォーカス以外が主対象の作業
- 文言調整やレイアウト微修正のみで、フォーカス移動・可視化・操作モデルに変更がない

## 出力契約
- 対象画面ごとに、該当SC（2.1.1/2.1.2/2.1.4/2.4.3/2.4.7/2.4.11）への適合状況を示す
- 不適合がある場合は、再現手順・原因・修正方針（実装パターン）をセットで示す
- 修正を行う場合は、キーボード操作手順と期待結果を含む最小テスト観点を併記する
