...
## このスキルを使う条件
- PR時点でアクセシビリティ回帰を検知したい、または検知漏れが続いている
- `lint` / コンポーネント検査 / E2E / 定期スキャンを、既存CIに段階導入したい
- 自動検査の結果を、マージ可否やリリース判定（Quality Gate）に結びつけたい

## このスキルを使わない条件
- CIを持たない単発調査で、現時点の監査レポート作成だけが目的
- 手動監査のみを求められており、自動検査やゲート運用を導入しない
- 既存CIのゲート方針が確定済みで、しきい値調整や運用変更が不要
