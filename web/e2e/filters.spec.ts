...
test.describe('Keyword Search', () => {
  test('search by keyword filters results', async ({ page }) => {
    ...
    await page.screenshot({ path: shotPath('search-01-initial.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('search-02-keyword-entered.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('search-03-results.png'), fullPage: true });

    // Clear search
    await searchInput.fill('');
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('search-04-cleared.png'), fullPage: true });
  });
  ...
  test('search with no results shows empty state', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: shotPath('search-05-no-results.png'), fullPage: true });
...
test.describe('Multiple Tags Filter', () => {
  test('AND mode - filters by all selected tags', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('tags-and-01-initial.png'), fullPage: true });

    await openFiltersDrawer(page);

    // Select first tag (ゲーム) from 大分類
    await selectTag(page, /^大分類から選ぶ/, 'ゲーム');
    await page.screenshot({ path: shotPath('tags-and-02-first-tag.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('tags-and-03-and-mode.png'), fullPage: true });

    await closeFiltersDrawer(page);
    await page.screenshot({ path: shotPath('tags-and-04-filtered.png'), fullPage: true });

    // Now switch to OR mode and add another tag
    await openFiltersDrawer(page);
    await page.locator('select').first().selectOption('OR');
    await page.screenshot({ path: shotPath('tags-and-05-or-mode.png'), fullPage: true });

    await closeFiltersDrawer(page);
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('tags-and-06-or-filtered.png'), fullPage: true });
  });
});
...
test.describe('Year Filter', () => {
  test('filter by year using histogram', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('year-01-initial.png'), fullPage: true });

    await openFiltersDrawer(page);
    await page.screenshot({ path: shotPath('year-02-drawer-open.png'), fullPage: true });
    ...
    await yearBar.click();
    await page.screenshot({ path: shotPath('year-03-year-selected.png'), fullPage: true });

    // Verify the year is highlighted
    await expect(yearBar).toHaveAttribute('data-on', 'true');

    await closeFiltersDrawer(page);
    await page.screenshot({ path: shotPath('year-04-filtered.png'), fullPage: true });
...
test.describe('Date Range Filter', () => {
  test('filter by today', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('date-01-initial.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('date-02-today-selected.png'), fullPage: true });

    await closeFiltersDrawer(page);
    await page.screenshot({ path: shotPath('date-03-today-filtered.png'), fullPage: true });

    // Clear date filter
    await openFiltersDrawer(page);
    const clearDateButton = page.locator('.dateTop').getByRole('button', { name: 'クリア' });
    await clearDateButton.click();
    await page.screenshot({ path: shotPath('date-04-cleared.png'), fullPage: true });
    await closeFiltersDrawer(page);
  });
  ...
  test('filter by date range using calendar', async ({ page }) => {
    ...
    await page.screenshot({ path: shotPath('date-05-calendar-initial.png'), fullPage: true });

    // Navigate to previous month
    await page.locator('.calHeader .btn').first().click();
    await page.waitForTimeout(200);
    await page.screenshot({ path: shotPath('date-06-prev-month.png'), fullPage: true });
    ...
    if (await dateWithVideo.isVisible()) {
      ...
      await page.screenshot({ path: shotPath('date-07-start-date.png'), fullPage: true });
      ...
      if (await secondDate.isVisible()) {
        ...
        await page.screenshot({ path: shotPath('date-08-end-date.png'), fullPage: true });
      }
    }

    await closeFiltersDrawer(page);
    await page.screenshot({ path: shotPath('date-09-range-filtered.png'), fullPage: true });
  });
});
...
test.describe('Duration Filter', () => {
  test('filter by duration range', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('duration-01-initial.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('duration-02-min-set.png'), fullPage: true });

    // Set maximum duration to 180 minutes
    await maxSlider.fill('180');
    await page.screenshot({ path: shotPath('duration-03-max-set.png'), fullPage: true });

    await closeFiltersDrawer(page);
    await page.screenshot({ path: shotPath('duration-04-filtered.png'), fullPage: true });

    // Reset duration filter
    await openFiltersDrawer(page);
    await page.locator('.rangeRow .btn').click();
    await page.screenshot({ path: shotPath('duration-05-reset.png'), fullPage: true });
    await closeFiltersDrawer(page);
  });
});
...
test.describe('Sort Order', () => {
  test('toggle sort order between newest and oldest', async ({ page }) => {
    ...
    await page.screenshot({ path: shotPath('sort-01-newest-first.png'), fullPage: true });

    // Switch to oldest first
    await page.getByRole('button', { name: '↑ 古い順' }).click();
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('sort-02-oldest-first.png'), fullPage: true });

    // Verify the button state changed
    const ascButton = page.getByRole('button', { name: '↑ 古い順' });
    await expect(ascButton).toHaveClass(/active/);
    await page.screenshot({ path: shotPath('sort-03-sorted.png'), fullPage: true });

    // Switch back to newest
    await descButton.click();
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('sort-04-back-to-newest.png'), fullPage: true });
  });
});
...
test.describe('Clear All Filters', () => {
  test('clear button resets all filters', async ({ page }) => {
    ...
    await page.screenshot({ path: shotPath('clear-01-initial.png'), fullPage: true });
    ...
    await closeFiltersDrawer(page);
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('clear-02-filter-applied.png'), fullPage: true });
    ...
    await page.getByRole('button', { name: '↑ 古い順' }).click();
    await page.waitForTimeout(200);
    await page.screenshot({ path: shotPath('clear-03-sort-changed.png'), fullPage: true });

    // Click clear button
    await page.getByRole('button', { name: 'クリア' }).click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: shotPath('clear-04-after-clear.png'), fullPage: true });
...
test.describe('Video Modal', () => {
  test('open modal and interact with video details', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('modal-01-initial.png'), fullPage: true });

    // Click first card
    const firstCard = page.locator('.grid .card').first();
    const cardTitle = await firstCard.locator('.title').textContent();
    await firstCard.click();
    await page.screenshot({ path: shotPath('modal-02-opened.png'), fullPage: true });
    ...
    await page.screenshot({ path: shotPath('modal-03-with-tags.png'), fullPage: true });

    // Click a tag in modal (should add to filter)
    if (tagCount > 0) {
      const firstTag = modalTags.first();
      await firstTag.click();
      await page.screenshot({ path: shotPath('modal-04-tag-clicked.png'), fullPage: true });
    }
    ...
    await page.screenshot({ path: shotPath('modal-05-url-copied.png'), fullPage: true });

    // Close modal
    await page.locator('#dlg').getByRole('button', { name: '閉じる' }).click();
    await page.screenshot({ path: shotPath('modal-06-closed.png'), fullPage: true });
  });
});
...
test.describe('Scroll and Load More', () => {
  test('scroll to bottom shows scroll to top button', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('scroll-01-top.png'), fullPage: true });

    // Scroll down
    await page.evaluate(() => window.scrollTo(0, 1000));
    await page.waitForTimeout(500);
    await page.screenshot({ path: shotPath('scroll-02-scrolled.png'), fullPage: true });

    // Check if scroll-to-top button appears
    const scrollTopBtn = page.locator('#scrollTopBtn');
    await expect(scrollTopBtn).toBeVisible();
    await page.screenshot({ path: shotPath('scroll-03-scroll-btn-visible.png'), fullPage: true });

    // Click scroll to top
    await scrollTopBtn.click();
    await page.waitForTimeout(500);
    await page.screenshot({ path: shotPath('scroll-04-back-to-top.png'), fullPage: true });
  });
  ...
  test('load more button loads additional items', async ({ page }) => {
    ...
    await page.screenshot({ path: shotPath('loadmore-01-initial.png'), fullPage: true });

    // Scroll to load more button if it exists
    const loadMoreBtn = page.locator('#moreBtn');
    if (await loadMoreBtn.isVisible()) {
      await loadMoreBtn.scrollIntoViewIfNeeded();
      await page.screenshot({ path: shotPath('loadmore-02-button-visible.png'), fullPage: true });

      await loadMoreBtn.click();
      await page.waitForTimeout(500);
      await page.screenshot({ path: shotPath('loadmore-03-more-loaded.png'), fullPage: true });
...
test.describe('Selected Tags Display', () => {
  test('selected tags appear below search bar', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoad(page);
    await page.screenshot({ path: shotPath('selected-01-initial.png'), fullPage: true });

    // Open drawer and select a tag
    await openFiltersDrawer(page);
    await selectTag(page, /^大分類から選ぶ/, 'ゲーム');
    await page.screenshot({ path: shotPath('selected-02-tag-selected.png'), fullPage: true });
    await closeFiltersDrawer(page);
    await page.waitForTimeout(300);
    ...
    await page.screenshot({ path: shotPath('selected-03-tag-displayed.png'), fullPage: true });

    // Click the tag to remove it
    await selectedTagsSection.locator('.chip', { hasText: 'ゲーム' }).click();
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('selected-04-tag-removed.png'), fullPage: true });
  });
});
...
test.describe('Tag Search in Drawer', () => {
  test('search filters tags in drawer', async ({ page }) => {
    ...
    await openFiltersDrawer(page);
    await page.screenshot({ path: shotPath('tagsearch-01-drawer-open.png'), fullPage: true });

    // Enter tag search
    const tagSearchInput = page.getByPlaceholder('例: 雑談 / ホラー / コラボ');
    await tagSearchInput.fill('ゲーム');
    await page.waitForTimeout(300);
    await page.screenshot({ path: shotPath('tagsearch-02-searched.png'), fullPage: true });
    ...
    await page.waitForTimeout(200);
    await page.screenshot({ path: shotPath('tagsearch-03-cleared.png'), fullPage: true });
