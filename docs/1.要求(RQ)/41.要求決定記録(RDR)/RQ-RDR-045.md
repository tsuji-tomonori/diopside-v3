...
| DR訓練（災害復旧訓練） | マルチリージョン構成を持たないため、DR訓練の対象構成が存在しない。[[RQ-AV-003-01]] の年次リストア検証で代替 | アーキテクチャ不適合＋規模不適合 |
...
| レートリミット/スロットリング | [[RQ-SH-002|利用者]]向けAPIを公開せず、管理APIは認証必須。レートリミットの追加統制は過剰 | アーキテクチャ不適合＋規模不適合 |
| DB性能チューニング（インデックス/クエリ最適化） | RDB不使用。DynamoDBはパーティションキー設計のみで性能確保 | アーキテクチャ不適合 |
| CDN最適化 | CloudFrontの標準キャッシュ設定で十分。[[RQ-SUS-002-01]] のCache-Control設定で実質充足 | 既存要件で充足 |

### 03.セキュリティ(SEC) — 不採用要件

| 参照要件 | 不採用理由 | 判断基準 |
|---------|-----------|---------|
| MFA/多要素認証 | ユーザーアカウント機能を持たない。[[RQ-SH-001|管理者]]認証はAWS IAMのMFAで対応済み | アーキテクチャ不適合 |
...
| SBOM（ソフトウェア部品表） | 個人プロジェクト規模でSBOM管理の運用コストが効果に見合わない。[[RQ-SEC-001-01]] の脆弱性スキャンで実質充足 | 規模不適合 |
| セキュリティリスクアセスメント（定期監査） | 個人運用で形式的な監査体制は過剰。[[RQ-SEC-001-01]] の受入基準で継続的な統制を維持 | 規模不適合 |
...
| 参照要件 | 不採用理由 | 判断基準 |
|---------|-----------|---------|
| PIIマスキング/匿名化 | [[RQ-PRC-001-01]] でPII非保存を方針化済み。マスキング対象データが存在しない | 既存要件で充足 |
...
| エラーバジェット/[[BD-INF-MON-002|SLO運用]] | 個人運用でエラーバジェットの消費判断・凍結運用は過剰。[[RQ-AV-001-01]] の可用率閾値で代替 | 規模不適合 |
...
| インシデント管理プロセス（PagerDuty等） | 個人運用で外部インシデント管理ツールは過剰。SNS通知＋[[RQ-OBY-003-01]] のランブックで代替 | 規模不適合 |
...
| 変更承認プロセス（複数人レビュー） | 個人開発でレビュアーが不在。[[RQ-DEV-001-01]] のCI自動チェックで品質ゲートを代替 | 規模不適合 |
...
| 参照要件 | 不採用理由 | 判断基準 |
|---------|-----------|---------|
| 非[[BD-DEV-ENV-002|本番環境]]のスケジュール停止 | サーバーレス構成でアイドルコストが発生しない。停止対象リソースが存在しない | アーキテクチャ不適合 |
| コスト異常検知（Cost Anomaly Detection） | [[RQ-COST-002-01]] のBudgetsアラートで段階検知が可能。Anomaly Detectionの追加価値が予算に見合わない | 予算制約＋既存要件で充足 |
...
| 参照要件 | 不採用理由 | 判断基準 |
|---------|-----------|---------|
| 追加WCAG項目（AAA水準等） | 既存24件（[[RQ-UX-001-01]]〜024）でWCAG 2.2 AA水準を網羅済み。AAA水準は過剰 | 既存要件で充足 |
...
| メッセージ重複排除（SQS/EventBridge） | メッセージキューを使用しない。[[RQ-DATA-002-01]] のバッチ冪等性で収集の重複を防止 | アーキテクチャ不適合＋既存要件で充足 |
...
| アイドル資源の自動排除 | [[RQ-SUS-001-01]] のサーバーレスファースト方針により、アイドル資源が構造的に発生しない | 既存要件で充足 |
...
- 移植を行わず既存34件のみで運用する案: バックアップ・暗号化・[[RQ-SEC-004-01|セキュリティヘッダー]]・冪等性等の実質的に価値のある要件が欠落し、運用リスクが残るため不採用。
