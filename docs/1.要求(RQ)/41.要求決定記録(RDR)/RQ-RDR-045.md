...
| エラーバジェット/[[BD-INF-MON-002|SLO運用]] | 個人運用でエラーバジェットの消費判断・凍結運用は過剰。[[RQ-AV-001]] の可用率閾値で代替 | 規模不適合 |
...
- 移植を行わず既存34件のみで運用する案: バックアップ・暗号化・[[RQ-SEC-004|セキュリティヘッダー]]・冪等性等の実質的に価値のある要件が欠落し、運用リスクが残るため不採用。
