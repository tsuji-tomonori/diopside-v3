...
- タグキーは `UpperCamelCase` を標準とし、`Environment` の許容値は `Production` / `Staging` / `Development` / `Test` に固定する。
...
## 影響
- 要求文書: [[RQ-COST-001]] と [[RQ-SEC-001]] を意味変更し、タグ統制の受入基準を追加する。
- 設計文書: [[BD-ADR-015]] と [[DD-COST-001]] に単一アカウント向けの強制/検知手段を反映する。
