...
## 決定事項
- DDD全量対応の戦略基盤として、5+1境界づけられたコンテキスト（Ingestion / TagManagement / Publishing / Viewing / Administration / Analytics）を要求上の正本境界として採用する。
- ドメイン境界の設計正本は `[[BD-SYS-DOM-001]]` とし、旧DOMディレクトリ文書は参照置換後に削除する。
...
- ドメインモデルでは、最低4集約（動画、[[RQ-GL-002|収集実行]]、[[RQ-GL-005|タグ辞書]]、[[RQ-GL-006|アーカイブ索引カタログ]]）について集約境界と集約ルートを明記し、境界横断更新を禁止する。
...
## 影響
- `[[BD-SYS-DOM-001]]` を要求追跡のドメイン境界参照先として利用する。
...
## 変更履歴
- 2026-02-15: ドメイン境界正本を `[[BD-SYS-DOM-001]]` へ移管し、旧DOM文書削除方針へ更新 [[RQ-RDR-040]]
- 2026-02-14: 決定記録から具体パス記述を除外し、要求レベルの契約表現へ統一 [[RQ-RDR-042]]
- 2026-02-14: 新規作成 [[RQ-RDR-040]]
