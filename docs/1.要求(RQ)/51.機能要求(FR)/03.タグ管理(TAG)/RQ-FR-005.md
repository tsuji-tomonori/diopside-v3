...
- 種別: 機能要求
- 優先度: MUST
- 要求: 本システムは、[[RQ-SH-001|管理者]]が管理画面から[[RQ-GL-005|タグ辞書]]を追加・更新・無効化し、DB正本へ反映できることを提供する。
- 根拠: タグ運用ルールを収集処理から分離し、用語追加時の運用変更を迅速に反映するため。
- 受入基準:
  - [[RQ-GL-005|タグ辞書]]更新はBackend API経由でDB正本に保存される。
  - DB正本更新後、配信用 `tag_master.json` 再生成を起動できる。
  - 公開切替後の `tag_master.json` は更新内容を反映し、公開前版との差分を追跡できる。
...
- 公開切替後に利用者向け `tag_master.json` が更新内容を返すこと。
...
- 2026-02-11: DB正本更新と配信用 `tag_master.json` 再生成の受入条件を追加 [[RQ-RDR-034]]
