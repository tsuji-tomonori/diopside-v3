...
- 種別: 機能要求
- 優先度: MUST
- 要求: 本システムは、管理画面運用の配信前提として、単一CloudFront Distribution上で `/web/*`（公開画面と管理画面直アクセス）、`/docs/*`（ドキュメント）、`/openapi/*`（OpenAPI）、`/api/v1/*`（業務API）をパスで分岐し、干渉なく配信できることを提供する。
- 根拠: 配信入口を単一化しつつ責務境界を固定し、運用負荷と経路競合リスクを抑えるため。
- 受入基準:
  - `/` アクセス時に `/web/` へ誘導される。
  - `/web/admin` 直アクセス時に管理画面を表示できる。
  - 公開UI（`/web/`）には管理画面への遷移ボタン/リンクを表示しない。
...
- `/web/admin` 直アクセスで管理画面を開けること。
- `/web/` 上に管理画面導線が表示されないこと。
- `/docs/*` の拡張子なしパスで文書表示できること。
- `/api/v1/*` と `/openapi/*` の未認証アクセスが拒否されること。

## 個別確認チェックリスト
- 確認主体: [[RQ-SH-001|管理者]]
- 重点確認対象: パス分岐境界、認証境界、公開UI/管理UIの導線境界
- 確認方法: CloudFront behavior設定、認証有無アクセス結果、版対応URLを照合する。

## 変更履歴
- 2026-03-08: 管理画面入口を `/web/admin` 直指定に限定し、公開UIへ管理導線を置かない受入基準を追加 [[RQ-RDR-055]]
