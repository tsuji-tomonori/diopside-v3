...
## 対象範囲
- [[RQ-GL-019|再確認実行]]、差分確認、手動タグ付け提案取込、公開反映、公開成果物更新確認。
- 一覧/検索など利用者向け導線は本シナリオ対象外とする。
...
## テストデータ
- 再確認差分が出る動画群と差分なし動画群を混在させる。
- JSON検証成功/失敗を再現できる提案データを用意する。
...
## 失敗時の切り分け導線
- 再確認失敗は [[DD-APP-API-012]] 実行ログとIT結果（[[IT-RST-001]]）へ戻す。
- JSON検証失敗は [[DD-APP-API-013]] のエラー明細を起点に入力データ側へ戻す。
- 公開反映失敗は [[DD-APP-API-015]] 状態遷移と [[AT-RUN-001]] を参照し、旧公開版維持で判定継続する。
