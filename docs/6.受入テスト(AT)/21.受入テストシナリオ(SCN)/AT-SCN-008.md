...
## 対象範囲
- 収集結果明細におけるメタデータ正規化判定と、Web操作の性能閾値判定。
- タグ管理の運用導線は本シナリオ対象外とする。
...
## テストデータ
- 公式チャンネル外データ、日時不正データ、欠損項目データ、出演補完対象データを含める。
- 性能計測では同一計測条件で複数回実行し、p95を算出可能なサンプル数を確保する。
...
- 非機能判定: 性能p95が[[RQ-PS-001-01]]の閾値を達成。
...
## 失敗時の切り分け導線
- 正規化不整合は [[DD-APP-API-011]] の明細を起点に収集処理へ戻し、IT結果（[[IT-RST-001]]）と照合する。
- 性能閾値未達は計測条件差異を除外後、FE/BEの局所性能課題としてUT証跡（[[UT-RPT-001]]）へ切り戻す。
...
## 変更履歴
- 2026-02-28: 共通テンプレ（対象範囲/前提条件/対応要求/テストデータ/記録/切り分け/参照UT-IT）へ章構成を統一
