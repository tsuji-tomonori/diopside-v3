...
## 対象範囲
- タグ作成/更新/廃止/統合、動画への手動タグ付け、公開反映後の整合確認。
- [[RQ-GL-019|再確認実行]]フロー自体は [[AT-SCN-007]] を対象とする。
...
## テストデータ
- 同義語を含む既存タグ、廃止候補タグ、統合候補タグ、手動付与対象動画を用意する。
- 公開反映後に `tag_master.json` / `archive_index` 差分確認ができる状態を用意する。
...
## 失敗時の切り分け導線
- タグ管理操作失敗は [[DD-APP-API-013]] 契約とIT結果（[[IT-RST-001]]）へ戻す。
- 公開反映不整合は [[DD-APP-API-015]] 状態遷移と公開成果物差分を確認する。
- 検索結果不整合はシノニム/統合設定のデータ整合を確認し、必要時に [[AT-SCN-002]] で再確認する。
...
## 変更履歴
- 2026-02-28: 共通テンプレ（対象範囲/前提条件/テストデータ/記録/切り分け/参照UT-IT）へ章構成を統一
