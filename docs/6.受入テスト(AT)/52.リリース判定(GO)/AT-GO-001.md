---
id: AT-GO-001
title: リリース判定 001
doc_type: リリース判定
phase: AT
version: 1.0.14
status: 下書き
owner: RQ-SH-001
created: 2026-01-31
updated: '2026-02-28'
up:
- '[[BD-DEV-TEST-001]]'
- '[[IT-PLAN-001]]'
related:
- '[[AT-RPT-001]]'
- '[[AT-RCHK-001]]'
- '[[AT-OPS-001]]'
- '[[AT-SCN-001]]'
- '[[AT-SCN-002]]'
- '[[AT-SCN-003]]'
- '[[AT-SCN-004]]'
- '[[AT-SCN-005]]'
- '[[AT-SCN-006]]'
- '[[AT-SCN-007]]'
- '[[AT-SCN-008]]'
- '[[AT-SCN-009]]'
- '[[RQ-DEV-007-01]]'
- '[[RQ-RDR-050]]'
tags:
- diopside
- AT
- GO
---


## 判定目的
- 受入結果をAPI単位で確認し、未解決リスクの有無を明示してGo/No-Goを決定する。

## 判定入力
- シナリオ結果: [[AT-RPT-001]]
- スコープ判定結果: [[AT-RCHK-001]], [[AT-RPT-001]] の「スコープ判定サマリ（記入テンプレート）」
- スコープ別テスト検証結果: [[AT-RPT-001]] の「スコープ別テスト検証（2026-02-28）」と `reports/scope_test_verification_2026-02-28.md`
- API逆引き結果: [[AT-RPT-001]] の「API単位判定記録（記入例/テンプレート）」
- 環境別運用値: [[AT-RPT-001]] の「環境別運用値判定記録（記入テンプレート）」
- 可観測性結果: [[AT-RPT-001]] の「可観測性判定記録（記入テンプレート）」
- UT/IT証跡: [[AT-RPT-001]] の「UT/IT実施証跡サマリ」
- 運用チェック結果: [[AT-OPS-001]]
- GitHub Actions証跡: 必須ステータスチェック結果、Environment承認履歴、Artifacts参照先（a11y含む）
- Issueラベル起動証跡: `issue_number`、実行者、付与ラベル、作成PR番号、実行結果コメント

## Go条件
- 必須シナリオ（[[AT-SCN-001]]〜[[AT-SCN-009]]）がすべてPass。
- スコープ判定（[[RQ-SC-001]]〜[[RQ-SC-008]]）がすべてPass、または受容済みConditionalである。
- API単位で [[DD-APP-API-002]]〜[[DD-APP-API-015]] が全てPassまたはリスク受容済み。
- 可観測性（通知遅延/記録率/欠測率）が [[RQ-OBY-001-01]] の閾値を満たす。
- 月次可用率とMTTRが [[RQ-AV-001-01]] の閾値を満たす。
- 性能p95が [[RQ-PS-001-01]] の閾値を満たす。
- 重大度Highの未解決不具合が0件。

## No-Go条件
- 必須シナリオFailが1件以上。
- スコープ判定で未受容のFail/Conditionalが1件以上。
- API単位で重大機能の未解決Failが残存。
- 可観測性のいずれかが閾値未達、または証跡不備で判定不能。
- 可用性/性能の判定不能または閾値未達が残存。
- [[AT-SCN-005|障害時対応シナリオ]]で復旧不能。

## 判定記録フォーマット
- 判定日
- 判定者
- シナリオPass率
- API Pass率
- 環境別運用値参照結果
- 可観測性参照結果
- 例外承認事項
- 最終判定（Go/No-Go）

## 非機能ゲート判定表（AT-RPTから転記）
| 項目 | 閾値 | 実測値 | 判定 | コメント |
| --- | --- | --- | --- | --- |
| 月次可用率 | 99.5%以上 |  |  |  |
| MTTR | 30分以内 |  |  |  |
| LCP p95 | 2.5秒以下 |  |  |  |
| 検索 p95 | 1.5秒以下 |  |  |  |
| [[RQ-GL-010|段階ロード]] p95 | 2.0秒以下 |  |  |  |
| 並び替え p95 | 1.0秒以下 |  |  |  |
| 通知遅延 | 5分以内 |  |  |  |
| ログ記録率 | 99%以上 |  |  |  |
| ログ欠測率 | 1%未満 |  |  |  |

## API判定転記表（AT-RPTから転記）
| API ID | AT-RPT判定 | 受容可否 | 判定コメント |
| --- | --- | --- | --- |
| [[DD-APP-API-002]] |  |  |  |
| [[DD-APP-API-003]] |  |  |  |
| [[DD-APP-API-004]] |  |  |  |
| [[DD-APP-API-005]] |  |  |  |
| [[DD-APP-API-006]] |  |  |  |
| [[DD-APP-API-007]] |  |  |  |
| [[DD-APP-API-008]] |  |  |  |
| [[DD-APP-API-009]] |  |  |  |
| [[DD-APP-API-010]] |  |  |  |
| [[DD-APP-API-011]] |  |  |  |
| [[DD-APP-API-012]] |  |  |  |
| [[DD-APP-API-013]] |  |  |  |
| [[DD-APP-API-014]] |  |  |  |
| [[DD-APP-API-015]] |  |  |  |

## スコープ最終判定転記表（AT-RCHK/AT-RPTから転記）
| スコープ | 対応Epic | AT-RCHK判定 | AT-RPT判定 | 受容可否 | 判定コメント |
| --- | --- | --- | --- | --- | --- |
| [[RQ-SC-001]] | 全体 |  |  |  |  |
| [[RQ-SC-002]] | EPIC-01 収集・正規化 |  |  |  |  |
| [[RQ-SC-003]] | EPIC-02 [[RQ-GL-005|タグ辞書]]・タグ整合 |  |  |  |  |
| [[RQ-SC-004]] | EPIC-03 配信成果物生成・公開 |  |  |  |  |
| [[RQ-SC-005]] | EPIC-04 一覧・検索・絞り込み |  |  |  |  |
| [[RQ-SC-006]] | EPIC-05 詳細・外部遷移 |  |  |  |  |
| [[RQ-SC-007]] | EPIC-06 運用（通知・結果・[[RQ-GL-011|再収集]]・再確認） |  |  |  |  |
| [[RQ-SC-008]] | EPIC-07 見どころ探索支援 |  |  |  |  |

## スコープ最終判定転記表（初期転記: 2026-02-28）
| スコープ | 対応Epic | AT-RCHK判定 | AT-RPT判定 | 受容可否 | 判定コメント |
| --- | --- | --- | --- | --- | --- |
| [[RQ-SC-001]] | 全体 | Pass | Pass | Yes | 必須シナリオと[[IT-CASE-001]]〜[[IT-CASE-013]]が全件Pass。 |
| [[RQ-SC-002]] | EPIC-01 収集・正規化 | Pass | Pass | Yes | [[IT-CASE-001]]/[[IT-CASE-002]]/[[IT-CASE-007]]/[[IT-CASE-010]]/[[IT-CASE-012]] で受入成立。 |
| [[RQ-SC-003]] | EPIC-02 [[RQ-GL-005|タグ辞書]]・タグ整合 | Pass | Pass | Yes | [[IT-CASE-004]]/[[IT-CASE-011]]/[[IT-CASE-013]] で受入成立。 |
| [[RQ-SC-004]] | EPIC-03 配信成果物生成・公開 | Pass | Pass | Yes | [[IT-CASE-003]]/[[IT-CASE-006]]/[[IT-CASE-011]]/[[IT-CASE-013]] で受入成立。 |
| [[RQ-SC-005]] | EPIC-04 一覧・検索・絞り込み | Pass | Pass | Yes | [[IT-CASE-003]]/[[IT-CASE-004]]/[[IT-CASE-005]] で受入成立。 |
| [[RQ-SC-006]] | EPIC-05 詳細・外部遷移 | Pass | Pass | Yes | [[IT-CASE-006]] がPass。 |
| [[RQ-SC-007]] | EPIC-06 運用（通知・結果・[[RQ-GL-011|再収集]]・再確認） | Pass | Pass | Yes | [[IT-CASE-007]]/[[IT-CASE-008]]/[[IT-CASE-012]]/[[IT-CASE-013]] で受入成立。 |
| [[RQ-SC-008]] | EPIC-07 見どころ探索支援 | Pass | Pass | Yes | [[IT-CASE-006]] と関連導線がPass。 |

## 再判定ゲート（No-Go解除条件）
- 必須再実行対象: [[AT-SCN-005]], [[AT-SCN-006]], [[AT-SCN-007]], [[AT-SCN-008]], [[AT-SCN-009]]
- 上記5件の実行結果が [[AT-RPT-001]] と [[AT-RCHK-001]] に反映済みであること。
- スコープ転記表で `受容可否=No` が0件であること。
- 未解決事項が残る場合は、期限・担当・影響範囲を明記した受容判断を添付すること。

## 判定サマリ（記入例）
| 項目 | 値 |
| --- | --- |
| 判定日 | 2026-02-28 |
| 判定者 | [[RQ-SH-001]] |
| シナリオPass率 | 9/9 |
| API Pass率 | 14/14 |
| 環境別運用値参照結果 | 開発: Pass / 本番: Pass |
| 可観測性参照結果 | 通知遅延/記録率/欠測率すべてPass |
| 例外承認事項 | なし |
| 最終判定 | Go |

## 変更履歴
- 2026-02-28: IT結合再実行（[[IT-CASE-001]]〜[[IT-CASE-013]] Pass）を反映し、スコープ最終判定を全件 Yes へ更新
- 2026-02-28: 判定入力にスコープ別テスト検証結果（`reports/scope_test_verification_2026-02-28.md`）参照を追加
- 2026-02-28: No-Go解除に向けた再判定ゲート（[[AT-SCN-005]]〜[[AT-SCN-009]]）を追加
- 2026-02-28: スコープ最終判定転記表へ初期転記（全体No-Go判定の根拠）を追加
- 2026-02-28: スコープ判定入力（[[AT-RCHK-001]]/[[AT-RPT-001]]）と最終転記表（[[RQ-SC-001]]〜[[RQ-SC-008]]）を追加
- 2026-02-28: 判定入力へUT/IT実施証跡サマリ参照を追加
- 2026-02-23: Issueラベル起動証跡（issue/actor/label/PR/結果コメント）を判定入力へ追加 [[RQ-RDR-050]]
- 2026-02-21: GitHub Actions証跡（必須チェック/承認履歴/Artifacts）を判定入力へ追加 [[RQ-RDR-050]]
- 2026-02-13: Go条件の必須シナリオを [[AT-SCN-009]] まで拡張し、API判定対象を [[DD-APP-API-015]] まで更新
- 2026-02-11: 非機能ゲート判定表と可観測性入力を追加し、Go/No-Go条件を具体化
- 2026-02-11: 環境別運用値（開発/本番）の参照入力を追加
- 2026-02-11: 判定条件・表内の文書ID参照をObsidianリンクへ統一
- 2026-02-10: 新規作成
- 2026-02-10: API逆引き前提のGo/No-Go条件を追加
- 2026-02-10: AT-RPT転記用のAPI判定表と記入例を追加
