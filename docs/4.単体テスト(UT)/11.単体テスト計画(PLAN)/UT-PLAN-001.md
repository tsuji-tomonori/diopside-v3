...
- 組合せ起因の回帰はペアワイズ法（2-wise）で補完し、因子定義は [[UT-PW-001]] と `UT-PW-*` 文書で管理する。
- 構造ベース技法（分岐網羅・条件網羅）は BEロジックとINFの判定分岐に適用し、[[UT-COV-001]] の閾値で監視する。
- 経験ベース技法（エラー推測）は過去障害・インシデント起点で補完し、再発防止ケースを領域別UTへ追加する。
- BEのI/F観点は OpenAPI 正本と `BD-APP-OAS-*` を基準にし、フロー起点の因子抽出は `DD-APP-API-*` を基準にする。
...
## 変更履歴
- 2026-03-06: BEのI/F基準を OpenAPI / `BD-APP-OAS-*`、フロー基準を `DD-APP-API-*` に分離
