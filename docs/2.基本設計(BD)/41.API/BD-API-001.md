...
- `[[RQ-FR-020]]` に対応する見どころ導線は、コメント密度波形の静的JSON配信で提供する。
...
- 動画詳細の補助表示データ（コメント密度波形）は `highlights/{videoId}.json` 契約で配信する。
...
| 静的JSON | `highlights/{videoId}.json` | 動画詳細のコメント密度波形と盛り上がり区間表示 | `[[RQ-FR-020]]`, `[[RQ-FR-022]]` |
...
- 2026-02-11: `[[RQ-FR-020]]` 意味変更に合わせ、コメント密度波形の静的配信契約（`highlights/{videoId}.json`）を追加
