...
- 検索画面（[[DD-UI-002|UI-U01]]/[[DD-UI-003|UI-U02]]）では一覧探索の連続性を最優先とし、[[DD-UI-004|UI-U03]] で補助情報を重ねても一覧コンテキストを破壊しない。
...
## 入力再利用とヘルプ導線
- [[RQ-UX-013]]: フィルタドロワー（[[DD-UI-003|UI-U02]]）を再表示した場合でも、直前に確定した[[RQ-GL-014|検索条件]]を初期値として復元する。
...
- **コンポーネント参照**: 実装は [[DD-UI-015|HighlightWavePanel]] を正本とする。
...
- **コンポーネント参照**: 実装は [[DD-UI-016|WordCloudPanel]] を正本とする。

## 変更履歴
- 2026-02-14: `UI-U01`〜`UI-U03` と `HighlightWavePanel`/`WordCloudPanel` の詳細設計リンクを追加 [[BD-ADR-018]]
- 2026-02-10: 新規作成
