...
flowchart TD
  subgraph U[利用者導線]
    U01[一覧画面 UI-U01 DD-UI-002]
    U02[検索・絞り込み UI-U02 DD-UI-003]
    U03[詳細モーダル UI-U03 DD-UI-004]
    UY[YouTube視聴ページ]
    UE1[詳細取得失敗表示]
    UE2[補助情報未生成表示]
  end
  ...
  subgraph A[管理者導線]
    A01[収集実行 UI-A01 DD-UI-007]
    A02[実行監視・履歴 UI-A02 DD-UI-006]
    A03[再収集設定 UI-A03 DD-UI-008]
    A04[配信前後確認・手動タグ UI-A04 DD-UI-009]
    A06[配信反映ジョブ UI-A06 DD-UI-010]
    A05[公開運用・配信経路確認 UI-A05 DD-UI-011]
    AE1[入力エラー/競合表示]
    AE2[失敗調査結果表示]
    AE3[配信反映失敗表示]
  end
...
## 遷移定義
- [[DD-UI-002|UI-U01]] -> [[DD-UI-004|UI-U03]]: カード選択で遷移し、戻り時は [[RQ-GL-014|検索条件]] とスクロール位置を復元する（[[RQ-UC-006]]）。
- `詳細モーダル -> YouTube`: 通常遷移と `t=<秒>` 遷移の2系統を持ち、失敗時はURLコピー導線を表示してモーダル継続利用を可能にする（[[RQ-FR-014]], [[RQ-FR-020]]）。
- [[DD-UI-007|UI-A01]] -> [[DD-UI-006|UI-A02]]: run ID 発行後に `queued`/`running` を監視し、完了時に `succeeded`/`failed` を確定表示する（[[RQ-FR-001]], [[RQ-UC-001]]）。
- [[DD-UI-006|UI-A02]] -> [[DD-UI-008|UI-A03]]: 失敗原因分類後、対象runを引き継いで [[RQ-GL-011|再収集]] を起動する（[[RQ-UC-007]], [[RQ-UC-008]]）。
- [[DD-UI-009|UI-A04]] -> [[DD-UI-010|UI-A06]]: 差分確認と手動タグ付け確定後、[[RQ-GL-018|配信反映実行]]を起動して静的成果物を再生成する（[[RQ-FR-005]], [[RQ-FR-019]]）。
- [[DD-UI-010|UI-A06]] -> [[DD-UI-011|UI-A05]]: 反映成功後に公開作業へ進み、経路異常時は監視画面へ戻す（[[RQ-UC-009]], [[RQ-FR-024]], [[RQ-FR-025]]）。
...
## 変更履歴
- 2026-02-14: 遷移ノードと遷移定義に各画面DD-UIリンク（`DD-UI-002`〜`DD-UI-011`）を追加 [[BD-ADR-021]]
