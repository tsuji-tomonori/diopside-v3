...
## 運用上の制約
- 本番変更は小差分で実施し、手順書と判定記録を同一変更で更新する（[[RQ-DEV-001-01]] 整合）。
- `'/openapi/*'` と `'/api/v1/*'` は未認証アクセスを拒否し、認証境界を崩さない（[[RQ-SEC-001-01]] 整合）。
- 可用率・MTTR・計画停止時間は [[RQ-AV-001-01]] の閾値を満たし、閾値未達時はGo/No-Go判定を保留する。
- 操作性能は [[RQ-PS-001-01]] の計測条件とp95閾値で継続監視し、劣化時は次リリースまでに是正する。
