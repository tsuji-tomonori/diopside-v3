...
- APIごとの契約Markdownは OpenAPI 正本から `[[BD-APP-OAS-000]]` / `BD-APP-OAS-*` を自動生成し、メソッド/パス/パラメータ/正常・異常レスポンスを表形式で配布する。
...
- `BD-APP-OAS-*` は成功レスポンスだけでなく、OpenAPI `responses` に定義された異常レスポンスも一致していることを確認する。
- 破壊的変更が検出された場合は、並行提供計画または新経路版の設計完了まで配信を停止する。

## 変更履歴
- 2026-03-07: `BD-APP-OAS-*` の配布対象を正常・異常レスポンスまで拡張し、契約検証で異常系整合も確認するよう更新 [[BD-SYS-ADR-023]]
