...
## 設計方針
- 本システムは[[RQ-SH-002|利用者]]向け参照を静的配信で提供し、管理画面の更新系APIと責務分離して管理する。
...
- 管理画面の更新系（タグ更新、反映トリガ、[[RQ-GL-011|再収集]]）は Backend API で受け、[[RQ-SH-002|利用者]]向け参照契約へ直接混在させない。
...
- クライアントは静的アセット取得失敗時に代替表示へフォールバックし、[[DD-APP-UI-004|詳細モーダル]]を継続表示する。

## 契約境界
- **[[RQ-SH-002|利用者]]向け参照契約**: 静的JSON/静的画像（本書で定義）。
...
- HTTP APIの operation 単位契約文書は OpenAPI 正本から `[[BD-APP-OAS-000]]` / `BD-APP-OAS-*` として自動生成し、メソッド/パス/パラメータ/戻り値を表形式で配布する。
- 処理フロー・制約・オラクルは `DD-APP-API-*` を参照し、HTTP I/F を手書きで再定義しない。
...
- **取得方式**: クライアントは[[DD-APP-UI-004|詳細モーダル]]表示時に動画IDからURLを組み立てて取得する。
- **HTTPステータス運用**:
  - `200`: 画像表示。
...
## 変更履歴
- 2026-03-06: OpenAPI 正本から生成する operation 単位契約文書（[[BD-APP-OAS-000]] / `BD-APP-OAS-*`）の位置づけを追加し、処理フローを `DD-APP-API-*` へ分離 [[BD-SYS-ADR-023]]
...
- 2026-02-11: [[RQ-SH-002|利用者]]向け参照契約にHTTP API共通方針の参照を追加 [[BD-SYS-ADR-023]]
