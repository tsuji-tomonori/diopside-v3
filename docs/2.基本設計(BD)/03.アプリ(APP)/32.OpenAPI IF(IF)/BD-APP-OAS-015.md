---
id: BD-APP-OAS-015
title: ドキュメント公開状態取得API契約
doc_type: API設計
phase: BD
version: 1.0.1
status: 下書き
owner: RQ-SH-001
created: 2026-03-06
updated: '2026-03-06'
up:
- '[[BD-APP-OAS-000]]'
- '[[BD-APP-API-004]]'
related:
- '[[DD-APP-API-014]]'
- '[[BD-APP-API-004]]'
- '[[BD-SYS-ADR-025]]'
- '[[UT-PLAN-005]]'
- '[[IT-PLAN-001]]'
tags:
- diopside
- BD
- API
- OpenAPI
openapi_operation_id: getDocsPublishRun
openapi_method: GET
openapi_path: /api/v1/admin/docs/publish/{docsPublishRunId}
---

## API概要
| 項目 | 値 |
| --- | --- |
| メソッド | `GET` |
| パス | `/api/v1/admin/docs/publish/{docsPublishRunId}` |
| operationId | `getDocsPublishRun` |
| タグ | `admin` |
| 認証/認可 | Cognito JWT + 管理者権限 |

## パラメータ
| 種別 | 名前 | 必須 | 型 | 既定値 | 制約 | 説明 |
| --- | --- | --- | --- | --- | --- | --- |
| path | docsPublishRunId | yes | string | - | format: uuid | 対象ドキュメント公開実行の識別子。 |

## リクエストボディ
- なし

## 戻り値
| ステータス | メディアタイプ | 型 | 制約 | 説明 |
| --- | --- | --- | --- | --- |
| 200 | application/json | object | - | Run status |

### 200 application/json フィールド
| フィールド | 必須 | 型 | 既定値 | 制約 | 説明 |
| --- | --- | --- | --- | --- | --- |
| docs_publish_run_id | yes | string | - | format: uuid | ドキュメント公開実行の識別子。 |
| status | yes | string | - | schema: PublishStatus; enum: queued \| running \| succeeded \| failed \| partial \| cancelled \| rolled_back | 現在の状態。 |
| steps | yes | array<object> | - | - | 処理ステップ一覧。 |
| steps[].name | yes | string | - | - | 処理ステップ名。 |
| steps[].status | yes | string | - | schema: PublishStatus; enum: queued \| running \| succeeded \| failed \| partial \| cancelled \| rolled_back | 処理ステップの状態。 |
| rollback | yes | object | - | - | ロールバック結果。 |
| rollback.executed | yes | boolean | - | - | ロールバックを実施したかどうか。 |
| rollback.rollback_to_version | no | string | - | - | ロールバック先の成果物バージョン。 |

## 変更履歴
- 2026-03-06: OpenAPI 正本から表形式のI/F文書を自動生成 [[BD-SYS-ADR-023]]
