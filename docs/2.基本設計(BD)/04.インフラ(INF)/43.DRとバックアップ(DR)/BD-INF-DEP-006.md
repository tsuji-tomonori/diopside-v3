...
## 目的
- 復旧優先度とRTO/RPOの判断基準をBDで固定し、障害時の意思決定を再現可能にする。
- DDでバックアップ設定値とRunbook詳細を確定するための根拠を引き渡す。
...
## DDへの引渡し（根拠/入力）
- コンポーネント別RTO/RPO表と優先復旧順序。
- DR戦略の採否理由（可用性目標、コスト制約、運用体制）。
- バックアップ対象一覧（方式、暗号化前提、保持方針、検証観点）。
- 演習計画（頻度、合否判定、記録先）。
...
## 未指定事項
- クロスリージョン/クロスアカウント複製の最終採否。
- バックアップ保持年限の最終値（法令/契約要件との整合）。
- 重大障害時の代替公開方針（縮退範囲、停止条件）。

## 受入基準
- 重要コンポーネントのRTO/RPOと復旧優先順位を説明できること。
- DDで具体設定（保持日数、スケジュール、復旧手順）を確定する入力が揃っていること。
...
## 変更履歴
- 2026-02-23: 目的/DD引渡し/未指定事項/受入基準を追加し、DR章をBD-DD分離テンプレートへ統一 [[BD-SYS-ADR-036]]
