...
## 目的
- [[RQ-OBY-001-01]] / [[RQ-AV-001-01]] / [[RQ-PS-001-01]] を、日次運用で再現可能な監視仕様へ落とし込む。
- 監視対象、観測点、SLI定義を固定し、判定と復旧判断の揺れを防ぐ。

## 必須設計項目（BDで必ず決める）
- 監視対象（収集run、配信経路、検索操作、認証境界、ログ品質、上流依存）の採用範囲。
- SLIの判定単位（日次/月次/時間）と異常時の分類基準。
- 欠測時の判定保留ルールと[[RQ-GL-011|再収集]]への接続条件。
- 通知レベル（WARN/CRITICAL）と一次対応の責務境界。

## DDへの引渡し（根拠/入力）
- 監視対象ごとの観測点一覧（指標、異常、一次対応）。
- SLI定義表（目標値、判定周期、算出前提）。
- 欠測運用と[[RQ-GL-011|再収集]]判定のルールセット（実装時の必須条件）。
...
| 性能 | 操作系p95（検索、詳細、配信反映確認） | [[RQ-PS-001-01]] 閾値内 | 日次 |
...
## 未指定事項
- 通知先ローテーションと営業時間外のエスカレーション境界。
- 監視欠測時に許容する再算出待機時間の上限。

## 受入基準
- 監視対象とSLIの対応関係を、運用担当が文書のみで追跡できること。
- DDでメトリクス実装とアラート設定を確定する入力が揃っていること。

## 変更履歴
- 2026-02-23: 必須設計項目/DD引渡し/未指定事項/受入基準を追加し、監視章を統一テンプレート化 [[BD-SYS-ADR-036]]
