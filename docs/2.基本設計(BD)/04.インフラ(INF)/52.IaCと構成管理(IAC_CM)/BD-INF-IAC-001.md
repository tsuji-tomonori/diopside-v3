...
## 目的
- IaC変更の統制境界と承認条件をBDで固定し、反映プロセスの再現性を確保する。
- DDで実行手順と検証条件を確定するための根拠を明確化する。
...
- 破壊的変更の定義、二重承認条件、適用時間帯制約。
- 承認者と実行者の責務分離、および証跡化ルール。
- 検証不成立時の完了禁止条件とロールバック必須条件。
...
| verify | [[IT-INFIT-SMK-001]], [[IT-INFIT-ROLL-001]] | 到達性確認、監視確認、切戻し判断 | 検証省略で完了扱い |

## 承認条件
- 破壊的変更（`cdk diff` 上のリソース置換、削除、認証境界変更）は二重承認を必須とし、業務時間外適用を禁止する。
- 非破壊変更でも `IT-INFIT-SMK-001` の成功証跡がない場合は完了扱いにしない。

## ロールバック
- 破壊的変更は事前にロールバック手順を定義し、適用前に検証済みであることを必須化する。

## 未指定事項
- 緊急変更時の承認代替フロー（不在時代理承認）。
- 変更凍結期間の適用条件。

## 受入基準
- 変更フローの各フェーズで必須出力と禁止事項が定義されていること。
