...
## 目的
- 環境分割の責務境界と昇格ルールを固定し、運用上の混線を防止する。
- DDでネットワーク/IAM/監査設定を具体化できる入力を揃える。
...
## DDへの引渡し（根拠/入力）
- 環境ごとの権限境界マトリクス（操作主体、許可操作、禁止操作）。
- 環境別タグ規約（必須キー、値ルール、例外承認条件）。
- 昇格フロー要件（承認者、証跡、差分確認）をDD手順へ引き渡す。
...
## 変更履歴
- 2026-02-23: 目的/DD引渡し/受入基準の章構成へ統一し、BDとDDの受け渡し項目を明確化 [[BD-SYS-ADR-036]]
