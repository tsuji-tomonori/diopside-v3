...
## 決定事項
- バッチ実行制約は `BD-ARCH-001` を正本として、全バッチの最大実行時間・再試行・同時実行数を定義する。
...
- CORS/レート制限/trace伝播/ログ日次エクスポートをAPI・監視・ログ設計に同時適用する。
...
## 影響
- [[BD-ARCH-001]]: BAT-006入出力契約、同時実行制御を追加。
- [[BD-API-002]] / [[DD-API-002]]: クォータ/レート制御、フォールバック、冪等性保存方式を追加。
- [[DD-API-014]]: docs公開失敗時ロールバック状態遷移を追加。
- [[BD-API-005]]: CORS/レート制限を追加。
- [[BD-MON-001]] / [[DD-LOG-001]]: クォータ監視、キュー滞留監視、trace伝播、30日保持補完を追加。
...
## 変更履歴
- 2026-02-13: 新規作成（バッチ制約/補助生成/クォータ制御/ロールバック運用の固定） [[BD-ADR-027]]
