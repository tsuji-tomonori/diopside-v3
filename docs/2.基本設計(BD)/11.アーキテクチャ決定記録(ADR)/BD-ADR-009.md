...
## 決定事項
- 動画詳細のワードクラウド表示は、動的生成APIではなく事前生成済みPNGの静的配信を採用する。
- 配信契約は `wordcloud/{videoId}.png` とし、クライアントが動画IDから取得先を決定する。
- 画像未生成・取得失敗・破損時は、詳細モーダル内で代替表示と再試行導線を提供する。
...
## 影響
- API設計: `[[BD-API-001]]` に静的画像配信契約、HTTPステータス運用、フォールバック方針を追加する。
- UI設計: `[[BD-UI-002]]` に表示位置・状態遷移・再試行導線を追加する。
- アクセシビリティ: `[[BD-UI-004]]` に代替テキスト・状態通知・キーボード操作要件を追加する。
