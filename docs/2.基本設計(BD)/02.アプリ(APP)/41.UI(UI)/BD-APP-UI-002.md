...
- 検索画面（[[DD-APP-UI-002|UI-U01]]/[[DD-APP-UI-003|UI-U02]]）では一覧探索の連続性を最優先とし、[[DD-APP-UI-004|UI-U03]] で補助情報を重ねても一覧コンテキストを破壊しない。
- [[RQ-FR-020]] の見どころ導線は、時刻一覧ではなく音波状のコメント密度可視化を採用し、区間クリックで遷移する。
- [[RQ-FR-021]] の[[RQ-GL-017|ワードクラウド]]表示は[[DD-APP-UI-004|詳細モーダル]]内の補助領域として定義し、主要操作（タグ切替・外部遷移・閉じる）を阻害しない。
...
- [[DD-APP-UI-004|詳細モーダル]]の情報階層は「必須情報（タイトル/メタ）」「操作導線（遷移/コピー/タグ）」「補助情報（[[RQ-GL-016|コメント密度波形]]/[[RQ-GL-017|ワードクラウド]]）」の順で構成する。
- [[RQ-GL-016|コメント密度波形]]は密度が高い区間を強調し、区間クリックで `t=<開始秒>` 遷移を実行する。
- [[RQ-GL-017|ワードクラウド]]は取得成功時のみ画像表示し、未生成・取得失敗・破損時は専用プレースホルダへ置換する。

## 入力再利用とヘルプ導線
- [[RQ-UX-013]]: [[DD-APP-UI-003|フィルタドロワー]]（[[DD-APP-UI-003|UI-U02]]）を再表示した場合でも、直前に確定した[[RQ-GL-014|検索条件]]を初期値として復元する。
- [[RQ-UX-013]]: 再入力が必要な項目（セッション切断等）は理由文言を表示し、再入力対象を最小項目へ限定する。
- [[RQ-UX-015]]: ヘルプ導線は「ヘルプ/お問い合わせ」で名称統一し、[[DD-APP-UI-002|一覧画面]]と[[DD-APP-UI-004|詳細モーダル]]で位置を変えない。
...
## [[DD-APP-UI-004|詳細モーダル]]表示仕様（[[RQ-GL-016|コメント密度波形]]）
- **表示条件**: `videoId` が解決済みで[[DD-APP-UI-004|詳細モーダル]]が開いたときに `highlights/{videoId}.json` を取得する。
...
- **再試行導線**: 取得失敗/不正データ時のみ再試行ボタンを表示し、[[DD-APP-UI-004|詳細モーダル]]を閉じずに再取得できる。
- **コンポーネント参照**: 実装は [[DD-APP-UI-015|HighlightWavePanel]] を正本とする。

## [[DD-APP-UI-004|詳細モーダル]]表示仕様（[[RQ-GL-017|ワードクラウド]]）
- **表示条件**: `videoId` が解決済みで[[DD-APP-UI-004|詳細モーダル]]が開いたときに `wordcloud/{videoId}.png` を取得する。
...
- **再試行導線**: 取得失敗/破損時のみ再試行ボタンを表示し、[[DD-APP-UI-004|詳細モーダル]]を閉じずに再取得できる。
...
- 2026-02-10: 新規作成
- 2026-02-11: UX要件（[[RQ-UX-003]]/[[RQ-UX-013]]/[[RQ-UX-015]]/[[RQ-UX-020]]）に対する入力再利用・ヘルプ導線・将来拡張時の代替導線要件を追加 [[BD-SYS-ADR-018]]
- 2026-02-11: 公開UIのモバイルファースト方針と端末別レイアウト仕様（390/768/1280px）を追加 [[BD-SYS-ADR-018]]
- 2026-02-11: [[RQ-FR-021]] 対応として[[DD-APP-UI-004|詳細モーダル]]の[[RQ-GL-017|ワードクラウド]]表示仕様とフォールバック挙動を追加 [[BD-SYS-ADR-009]]
- 2026-02-11: [[RQ-FR-020]] 意味変更に合わせ、[[RQ-GL-016|コメント密度波形]]の表示仕様と区間クリック遷移を追加 [[BD-SYS-ADR-010]]
