...
## 設計方針
- 品質特性は App/Infra と BE/FE の責務境界を明示した上で、[[RQ-SH-002|利用者]]体感（表示速度・操作継続性）と運用安定性（配備・監視・復旧）を一体で設計する。
- 体感性能の最大要因である「ウォーターフォール削減」と「クライアントJS削減」を優先し、Server Components 標準運用を維持する。
- [[BD-DEV-ENV-001|開発環境]]計測だけで合否を判断せず、`next build` / `next start` とフィールド指標（Web Vitals）を併用して判定する。
...
- 2026-02-11: [[RQ-UX-023-01]] 対応としてアクセシビリティ不具合の受付SLAと是正プロセス品質を追加 [[BD-SYS-ADR-024]]
