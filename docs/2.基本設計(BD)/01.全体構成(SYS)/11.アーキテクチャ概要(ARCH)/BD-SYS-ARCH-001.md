...
- [[RQ-SH-002|利用者]]体験は「高速初期表示」と「高精度絞り込み」を優先し、[[RQ-GL-010|段階ロード]]を採用する。
- 原本データはDBで一元管理し、[[RQ-SH-002|利用者]]向け参照は配信用静的JSONを正規経路とする。
...
| Web App | [[RQ-GL-010|段階ロード]]、クライアント検索、絞り込み、詳細表示。 | 配信JSON -> [[RQ-SH-002|利用者]]表示 | プレゼンテーション層 |
...
## [[BD-SYS-ARCH-002|論理構成]]マッピング
- [[BD-SYS-ARCH-002|論理構成]]の正本は [[BD-SYS-ARCH-002]] とし、以下の対応で本書の論理コンポーネントを4区分へ写像する。

| [[BD-SYS-ARCH-002|論理構成]]（[[BD-SYS-ARCH-002]]） | 対応コンポーネント（本書） | マッピング方針 |
|---|---|---|
| フロントエンド | Web App, Admin Console/Runbook | [[RQ-SH-002|利用者]]表示と運用者操作のUI境界を担う。 |
...
- 2026-02-15: 論理コンポーネントを表形式へ再編し、[[BD-SYS-ARCH-002]] への[[BD-SYS-ARCH-002|論理構成]]マッピングを追加 [[BD-SYS-ADR-033]]
- 2026-02-15: [[BD-SYS-ARCH-002|論理構成]]正本（[[BD-SYS-ARCH-002]]）への参照を追加し、4区分とバックエンド内処理形態分離を明確化 [[BD-SYS-ADR-033]]
...
- 2026-02-11: DB正本化と3層責務境界、将来[[DD-APP-API-006|検索API]]拡張境界を追加 [[BD-SYS-ADR-021]]
...
- 2026-02-10: ステークホルダー2者（[[RQ-SH-001|管理者]]/[[RQ-SH-002|利用者]]）に合わせて主体表現を修正 [[BD-SYS-ADR-001]]
