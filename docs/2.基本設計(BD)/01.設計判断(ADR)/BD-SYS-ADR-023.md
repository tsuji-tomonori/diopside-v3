...
- OpenAPI正本から operation 単位の API契約Markdown（`BD-APP-OAS-*`）を自動生成し、メソッド/パス/パラメータ/正常・異常レスポンスを表形式で配布する。
- `BD-APP-OAS-*` は operation ごとに適用される異常系を省略せず列挙し、`401/403/400/404/409/429/500` を適用条件に応じて OpenAPI `responses` へ定義する。
...
- 自動生成文書: `task api:docs:generate` / `task api:docs:check` を追加し、`BD-APP-OAS-*` を OpenAPI 正本から再生成し、異常系レスポンスの追記漏れを検知する。
...
## 変更履歴
- 2026-03-07: operation 単位契約文書で異常系レスポンスの列挙を必須化し、OpenAPI `responses` へ適用異常系を定義する運用を追加 [[BD-SYS-ADR-023]]
