...
## 影響
- デプロイ設計: [[BD-INF-DEP-003]] に公開フロー全体、実行チェーン、失敗時確認観点を追加する。
- 詳細設計: [[DD-INF-DEP-001]] に `siteAssetPath` 解決、Function rewrite、invalidationの詳細を反映する。
- 受入運用: [[AT-REL-001]] / [[AT-RUN-001]] に手順と障害切り分けを反映する。
- 段階導入: docs公開（Phase 1）を先行し、単一CloudFront分岐（Phase 2）は [[BD-SYS-ADR-014]] / [[BD-INF-DEP-004]] で拡張する。
