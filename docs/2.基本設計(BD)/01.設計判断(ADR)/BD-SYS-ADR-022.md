...
- 共通スキーマを固定することで、障害調査、通知判定、[[AT-GO-001|リリース判定]]で同一キーを利用できる。
- 30日保持は [[RQ-COST-001-01]] の上限制約と整合し、継続運用コストを抑制できる。

## 影響
- [[BD-INF-MON-001|監視設計]]: [[BD-INF-MON-001]] / [[BD-INF-MON-002]] で30日検索可能期間を前提に運用手順を維持する。
