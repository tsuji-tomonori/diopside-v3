...
- 画面種別は `window.location.pathname` を正本に判定し、`/web/admin` のみ管理UIを描画する。
...
- ルート状態: `pathname=/web/admin` のとき `admin`、それ以外の `/web/*` は `public` として扱う。
...
- 公開UIヘッダには管理UIへの遷移ボタン/リンクを配置しない。
- キーボード操作は `Tab` 順序と `Esc` 閉じるを全モーダルで統一する。

## I/Oまたは責務
- 入力: 現在URL、配信JSON、管理APIレスポンス、[[RQ-SH-002|利用者]]/[[RQ-SH-001|管理者]]操作イベント。
- 出力: 画面状態、描画イベント、エラー/再試行導線、監査対象操作ログ。

## 変更履歴
- 2026-03-08: `pathname` ベースの公開UI/管理UI分岐と、公開UI無導線の操作制約を追加 [[BD-SYS-ADR-045]]
