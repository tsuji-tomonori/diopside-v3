...
## 目的
- [[RQ-GL-002|収集実行]]の進捗と結果を[[RQ-SH-001|管理者]]が追跡できるようにする。

## エンドポイント
- `GET /api/v1/ops/ingestion/runs/{runId}`

## パスパラメータ
- `runId`: [[RQ-GL-002|収集実行]]ID（外部契約キー名）
...
2. 実行ストアから対象の[[RQ-GL-002|収集実行]]を取得する。
3. [[RQ-GL-002|収集実行]]が存在しない場合は `404 RUN_NOT_FOUND` を返す。
4. 実行中の[[RQ-GL-002|収集実行]]は進捗メトリクスを更新し、`processed/succeeded/failed` を再計算する。
5. 完了した[[RQ-GL-002|収集実行]]は `finishedAt` と `errorSummary` を確定値で返す。
6. 応答時に `traceId` と `retryable` を付与する。

## エラーマッピング
| エラーコード | HTTPステータス | 意味 |
| --- | --- | --- |
| `UNAUTHORIZED` | 401 | JWTが未指定または無効で[[RQ-GL-002|収集実行]]状態を照会できない。 |
| `RUN_NOT_FOUND` | 404 | 指定 `runId` の[[RQ-GL-002|収集実行]]が存在せず、状態を返せない。 |
| `RUN_STATE_UNAVAILABLE` | 503 | 実行ストアまたは進捗集計基盤の一時障害で状態を算出できない。 |
| `INTERNAL_ERROR` | 500 | サーバ内部例外により状態取得処理が中断した。 |

## [[BD-APP-API-003|エラーモデル]]
- `RUN_NOT_FOUND`
...
## 変更履歴
- 2026-02-19: 説明文の `run` 表記を [[RQ-GL-002|収集実行]] へ統一し、識別子 `runId` は契約キーとして明示
