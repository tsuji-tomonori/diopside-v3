...
| リソース | 主設定値 | 定義場所 | 根拠 |
|---|---|---|---|
| VPC/Subnet/Route | CIDR、サブネット分割、ルート | `network-stack` | [[DD-NET-001]] の具体値をCDKで再現し、環境差分を明示管理するため。 |
| Security Group/NACL | ingress/egress、denyルール | `network-stack` | 最小到達性と出口制御をIaCで固定し、手動逸脱を防ぐため。 |
| IAM Roles/Boundary | ロール名、権限境界、条件句 | `iam-stack` | [[DD-IAM-001]] の最小権限要件をコード化し、監査可能性を担保するため。 |
| Logs/Alarm | 保持日数、閾値、通知先 | `observability-stack` | [[DD-OBS-001]] の監視閾値をデプロイと同一ライフサイクルで管理するため。 |
| Deployment Pipeline | 承認ゲート、artifact ID | `cicd-stack` | [[DD-CICD-INF-001]] の破壊的変更制御を自動実施するため。 |
