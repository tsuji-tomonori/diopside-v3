...
- [[BD-INF-NET-001|ネットワーク境界方針]]は [[BD-INF-NET-001]] を正本とし、本書は方針を受けた実装値を正本化する。
...
## 1. VPC・サブネット設計
| リソース | 設定項目 | dev | prod | 根拠 |
|---|---|---|---|---|
| VPC | CIDR | `10.20.0.0/16` | `10.22.0.0/16` | 環境分割（[[BD-INF-ENV-001]]）と境界分離（[[BD-INF-NET-001]]）を満たすため、環境間で非重複CIDRを固定する。 |
| Public subnet | CIDR | `10.20.0.0/24` | `10.22.0.0/24` | 公開受信面（`443/tcp`）と出口制御の起点を分離するため。 |
| Private app subnet | CIDR | `10.20.10.0/24` | `10.22.10.0/24` | API/実行基盤を公開面から分離し最小到達性を維持するため。 |
| Private data subnet | CIDR | `10.20.20.0/24` | `10.22.20.0/24` | データ面への直接到達を抑止し、内部アクセスのみ許可するため。 |
...
| App SG | Egress | `443/tcp` のみ許可、送信先はAWS API/監視/認証基盤allowlist | 出口制御の原則（[[BD-INF-NET-001]]）を具体化するため。 |
...
- CIDR変更は破壊的変更として扱い、[[DD-INF-IAC-001]] の二重承認フローを適用する。

## 検証要件
- [[IT-INFIT-NET-001]] で許可経路/拒否経路の到達性テストを実施する。
