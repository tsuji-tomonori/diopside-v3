...
- 配備は `docs -> front -> api -> infra verify` の順で実行する。
...
3. apiを配備し、`/api/v1/ops/diagnostics/health` 応答を確認する。
...
| `/api/v1/ops/diagnostics/health` | 200でヘルス応答 | api再配備 |

## ロールバック条件
- docs/front: 反映後5分以内に主要導線が復帰しない場合は直前版へ切戻す。
- api: 互換性エラーが継続する場合は直前API版へ戻す。
...
## 変更履歴
- 2026-02-28: API正本化に合わせて領域分割配備手順の backend 表記を api へ更新 [[BD-SYS-ADR-036]]
