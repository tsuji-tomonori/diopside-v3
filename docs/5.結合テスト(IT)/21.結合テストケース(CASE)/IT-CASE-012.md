...
## 契約化した受入条件
- 障害検知APIが障害種別を `checks[]` へ反映する。
- 再[[RQ-GL-002|収集実行]]により欠損データの再生成が可能である。
- 復旧後の `health` が改善状態へ遷移する。

## 因子（機能要件ベース）
| 因子 | 関連要求 | 水準 |
| --- | --- | --- |
| 障害種別 | [[RQ-FR-018]] | ページ欠損, 鮮度遅延, タグ不整合 |
| 再[[RQ-GL-002|収集実行]] | [[RQ-FR-016]] | 成功, 失敗 |
| 復旧確認 | [[RQ-UC-007]] | 実施, 未実施 |
...
## 前提条件
- 結合環境で意図的な障害状態を再現できる。
- [[DD-APP-API-009|運用診断API]]が稼働している。
...
## 変更履歴
- 2026-02-28: UC基準フォーマットへ再編し、障害復旧契約のFR因子を追加
