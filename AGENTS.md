...
- BD文書の `## 変更履歴` 各行には、関連ADRリンク（`[[BD-*-ADR-xxx]]` 形式。例: `[[BD-SYS-ADR-034]]`）を含める
...
## 9. 信頼性運用ルール（失敗率/長時間セッション対策）
- `apply_patch` / `edit` の直前に、対象パスを `glob` または `read` で存在確認する。
- セッション中に rename/move が起きた場合、旧パスのまま編集せず最新パスを再解決してから編集する。
- 同一対象で編集失敗が連続した場合は、次の順で復旧する。
  1. `glob` / `grep` で再探索して対象を再特定する。
  2. 差分範囲を縮小して再実行する（最小差分）。
  3. 長い手順は小タスクに分割して順次適用する。
- サブエージェントは 15 分を目安にタイムボックス運用し、応答停止時は打ち切って要因を記録し再計画する。
- 30-60 分を超える作業はチェックポイント（到達状態・未完了・再開手順）を残し、長時間ハングを避ける。
