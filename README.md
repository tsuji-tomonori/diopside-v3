...
`docs:guard` / `docs:check` では、関連変更がある場合に
`BD-INF-DEP-005` の管理対象AWSリソース一覧と prod 条件の `cdk synth` を比較し、
過不足を `reports/infra_resource_check.md` へ出力します。
...
インフラ資源台帳と `cdk synth` の個別照合:

```bash
task docs:infra:check
```
...
- `docs:infra:check` は `infra/test/fixtures/site` を使って prod 条件の `cdk synth` を行うため、Quartz build と AWS 認証は不要です。
