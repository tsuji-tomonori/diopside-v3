...
- 実施:
  - `BD-ADR-009` を新規追加し、[[RQ-GL-017|ワードクラウド]]の配信方式を「事前生成の静的画像配信」として決定。
...
- 実施:
  ...
  - `DD-COST-001` を意味変更し、[[RQ-GL-005|タグ辞書]]、強制/検知フロー、Billing運用、是正SLAを具体化。
- 影響確認:
...
- 対象: `RQ-UX-001`, `RQ-UX-005`, `RQ-RDR-032`, `BD-UI-002`, `DD-UI-002`, `BD-ADR-020`
- 実施:
  ...
  - `BD-UI-002` にモバイルファースト設計方針、端末別レイアウト仕様、状態遷移一貫性要件を追加。
  - `DD-UI-002` を詳細化し、端末別レイアウト、コンポーネント責務、状態遷移、操作制約を実装可能粒度で定義。
  - 設計決定記録 `BD-ADR-020` を新規作成し、モバイルファースト採用理由と却下案を明文化。
- 影響確認:
  - 要求整合: `RQ-RDR-032 -> RQ-UX-001/RQ-UX-005` で端末優先順位の判断根拠と受入条件が追跡可能。
  - 設計整合: `BD-ADR-020 -> BD-UI-002 -> DD-UI-002` の経路で、設計意図と実装仕様の接続を確認。
  - 適用境界整合: 方針適用を公開UIに限定し、管理画面への過剰適用を回避。

## 追記（環境定義の2層化と一時検証方針の設計反映）
- 対象: `BD-ENV-001`, `BD-ENV-002`, `BD-ADR-018`
- 実施:
  - 環境区分を「開発」「本番」の2層として明文化し、`BD-ENV-001` と `BD-ENV-002` の本文を環境差分・依存・運用制約ベースへ更新。
  - 常設の独立検証環境は持たず、本番準拠の一時環境で検証する運用方針を両ENV文書へ追記。
  - 意味変更の設計決定として `BD-ADR-018` を新規作成し、2層化の採用理由と影響範囲を固定。
- 影響確認:
...
- 対象: `BD-ADR-020`
- 実施:
  - `BD-ADR-020` を新規作成し、タグ原本のDB正本化と利用者向けJSON配信分離を設計判断として記録。
  - 更新イベント起点/定期バッチでの再生成を採用し、利用時点での都度再生成を不採用とする理由を明記。
- 影響確認:
  - 設計整合: `RQ-RDR-031 -> BD-ADR-020 -> BD-API-001/BD-DATA-001` の追跡経路を構築。
...
- 対象: `RQ-FR-005`, `RQ-FR-009`, `RQ-UC-004`, `RQ-GL-005`, `BD-API-001`, `BD-DATA-001`, `DD-API-001`, `DD-API-005`, `DD-API-011`, `IT-CASE-009`, `AT-SCN-007`, `BD-ADR-018`, `BD-UI-002`
- 実施:
  - `RQ-FR-005/009`, `RQ-UC-004`, `RQ-GL-005` を更新し、[[RQ-GL-005|タグ辞書]]の境界を「DB正本 + 生成JSON配信」へ統一。
  - `BD-API-001`, `BD-DATA-001` を更新し、`tag_master.json` の生成トリガー/公開切替/失敗時挙動とタグ正本管理層を追記。
  - `DD-API-001`, `DD-API-005` を更新し、公開契約と運用契約（更新/公開）を分離。
  - `DD-API-011` を新規追加し、[[RQ-GL-005|タグ辞書]]更新APIと公開APIの契約を定義。
  - `IT-CASE-009`, `AT-SCN-007` を新規追加し、更新から公開反映までの結合/受入観点を追加。
  - 既存参照競合を解消するため `BD-ADR-018` を新規追加し、`BD-UI-002` のモバイル方針参照を修正。
- 影響確認:
  - 要求整合: `RQ-RDR-031` と `BD-ADR-020` で定義した責務境界がFR/UC/GLへ反映された。
  - 設計整合: `RQ-RDR-031 -> BD-ADR-020 -> BD-API-001/BD-DATA-001 -> DD-API-001/005/011` の追跡経路を構築。
  - テスト整合: `DD-API-011 -> IT-CASE-009 -> AT-SCN-007` の検証経路で運用更新から利用者反映までを確認可能。

## 追記（BD-UIの画面一覧/画面遷移を実体化）
- 対象: `BD-UI-001`, `BD-UI-003`
- 実施:
  - `BD-UI-001` をテンプレート記述から再構成し、利用者導線（一覧/検索/詳細）と管理者導線（収集実行/監視/[[RQ-GL-011|再収集]]/配信前後確認/公開運用）の画面カタログを追加。
  - 各画面に対して主体、主目的、入口、主要操作、主な状態、関連要求/UCを表形式で定義し、レビュー可能な設計粒度へ更新。
  - `BD-UI-003` に利用者導線/管理者導線を分離したMermaid遷移図、遷移定義、例外復帰方針を追加。
  - 両文書の `related` を `RQ-UC-001/002/006/007/008/009` および `BD-ADR-009/010` へ補強し、`updated` と `version`（PATCH）を更新。
- 影響確認:
  - 要求整合: `RQ-UC` 群の操作フローが `BD-UI` 側で画面単位に受けられる構造へ整理され、画面責務の曖昧さを解消。
  - 設計整合: `BD-ADR-009/010` で定義した補助表示方針（波形/[[RQ-GL-017|ワードクラウド]]）を、画面遷移上の補助遷移として反映。
...
- 実施:
  - `AT-PLAN-001` に、環境定義（[[BD-ENV-001]]/[[BD-ENV-002]]）を正本とした環境別運用値の記録方針を追加。
...
- 影響確認:
  - 設計整合: `BD-ENV-001` / `BD-ENV-002` の運用制約をAT文書上で判定可能な記録項目へ変換できた。
...
## 追記（監視チェーン文書の全面改訂）
- 対象: `BD-MON-001`, `BD-MON-002`, `DD-LOG-001`, `DD-AV-001`, `DD-PERF-001`, `AT-OPS-001`, `AT-RPT-001`, `AT-GO-001`
- 実施:
  - `BD-MON-001` を監視対象/観測点/SLI定義中心へ再構成し、欠測時の扱いと `[[RQ-GL-011|再収集]]` 導線を明文化。
  - `BD-MON-002` をSLO運用文書として再定義し、目標値・エラーバジェット・アラートレベル・逸脱時フローを追加。
  - `DD-LOG-001` にログカテゴリ、必須スキーマ、記録率・欠測率判定、保持方針を追加。
  - `DD-AV-001` に可用率/MTTRの算出式、障害分類、一次復旧判定フローを追加。
  - `DD-PERF-001` に計測条件、p95算出ルール、劣化時運用を追加。
...
## 追記（ビルド方針をデプロイ単位で分離）
- 対象: `BD-BUILD-001`, `BD-ADR-019`
- 実施:
  - `BD-BUILD-001` を更新し、ビルド責務を docs/web/api/infra の4つのデプロイ単位で分離して明記。
  - 単位ごとに成果物、配信経路、単位固有ゲート、失敗時の扱いをマトリクス化。
  - 共通TypeScript品質ゲートと、インフラ決定性ゲート（`cdk synth`、`cdk.context.json`、`cdk-nag`）を単位設計と接続。
  - 設計判断として `BD-ADR-019` を新規作成し、単位分離の採用理由と却下案を記録。
- 影響確認:
  - 設計整合: `BD-ADR-019 -> BD-BUILD-001` の追跡経路を追加し、ビルド失敗条件の責務境界を明確化。
  - 配信整合: `BD-DEP-003` / `BD-DEP-004` の経路分離（`/docs/*`, `/web/*`, `/openapi/*`, `/api/v1/*`）とビルド単位の対応を一致化。
  - 運用整合: 単位別失敗時に対象単位のみ再実行する方針を明記し、小差分リリース時の復旧時間短縮を狙う。

## 追記（デプロイ責務の領域分割明記）
- 対象: `BD-DEP-004`, `BD-DEP-005`, `BD-DEP-006`, `DD-DEP-002`, `DD-DEP-003`, `DD-DEP-004`, `AT-REL-001`
- 実施:
  - デプロイ文書を領域別（docs/infra/front/backend）に分割し、frontとbackendのBD/DD文書を新規追加。
  - `BD-DEP-004` をハブ化し、領域別の基本設計/詳細設計/運用確認の参照マップを追加。
  - `DD-DEP-002` をinfra正本として整理し、docs/front/backend詳細への参照境界を明確化。
  - `AT-REL-001` に領域別確認手順を追加し、配信時の切り分け先を固定。
- 影響確認:
  - 設計整合: `BD-DEP-004 -> (BD-DEP-005/006) -> (DD-DEP-003/004)` の追跡経路を追加し、責務境界を文書で分離。
  - 運用整合: 受入時に領域単位でPass/Fail判定でき、障害時の一次切り分けを短縮可能。
  - 要求整合: `RQ-FR-025` の経路分岐要求を、領域別のデプロイ責務として追跡可能化。

## 追記（Phase 2: DDL/DB制約詳細の実体化）
- 対象: `DD-DDL-001`〜`DD-DDL-008`, `DD-DBCON-001`, `DD-DBCON-002`
- 実施:
  ...
  - 移行設計 `DD-MIG-001` と接続し、制約追加時の backfill/contract 条件を明文化。
- 影響確認:
  - 設計整合: `DD-DDL-001 -> DD-DDL-002..008 -> DD-DBCON-001/002` の追跡経路を構築。
  - 運用整合: 制約違反時のロールバックとエラー分類（`DD-ERR-001`）を接続。
  - 受入整合: 移行時に件数整合/重複0件/必須欠損0件を判定可能化。

## 追記（Phase 3: UI/アルゴリズム/実装規約の実体化）
- 対象: `DD-UI-001`, `DD-UI-003`, `DD-UI-004`, `DD-UI-005`, `DD-UI-006`, `DD-ALG-001`, `DD-COMP-001`, `DD-COMP-002`, `DD-CODE-001`, `DD-REV-001`
- 実施:
...
- 影響確認:
  - 設計整合: `BD-UI-*` / `RQ-FR-*` / `DD-UI-*` / `DD-ALG-001` の接続を補強。
  - 運用整合: 管理画面操作の監査導線を `DD-UI-006` で明文化。
  - 品質整合: 実装規約 `DD-CODE-001` とレビュー規約 `DD-REV-001` の判定基準を統一。

## 追記（Phase 1: 非機能詳細設計の実体化）
- 対象: `DD-SEC-001`, `DD-ERR-001`, `DD-SCALE-001`, `DD-MIG-001`, `BD-ADR-020`
- 実施:
  - `DD-SEC-001` を更新し、認証/認可境界、入力検証、秘密情報管理、監査ログ、障害時ハンドリングを具体化。
  - `DD-ERR-001` を更新し、コード体系、HTTPマッピング、再試行ポリシー、ログ連携ルールを追加。
  - `DD-SCALE-001` を更新し、スケール対象、適用トリガ、縮退方針、監視判定を明文化。
  - `DD-MIG-001` を更新し、段階移行（expand/backfill/switch/contract）、互換期間、ロールバック条件を追加。
  - 設計決定として `BD-ADR-020` を新規作成し、非機能DDの必須記載項目と判定軸を統一。
- 影響確認:
  - 設計整合: `RQ-RDR-023 -> BD-ADR-020 -> DD-SEC/ERR/SCALE/MIG` の追跡経路を追加。
...
- 対象: `RQ-RDR-034`, `BD-ADR-021`, `BD-ARCH-001`, `BD-DATA-001`, `BD-API-001`, `BD-API-002`, `BD-UI-001`, `BD-DEP-004`, `DD-API-001`, `DD-API-005`, `AT-SCN-006`
- 実施:
  - `RQ-RDR-034` を新規作成し、原本データのDB正本化、利用者検索の静的JSON継続、将来API検索の段階導入を要求決定として記録。
  - `BD-ADR-021` を新規作成し、更新系（管理画面->Backend API->DB）と参照系（利用者UI->S3静的JSON）の責務境界を設計決定として固定。
  - `BD-ARCH-001` を更新し、3層境界（プレゼンテーション/アプリケーション/データ）と更新系・参照系の分離フローをMermaidへ反映。
  - `BD-DATA-001` を更新し、DB正本層/配信生成層/配信公開層のデータライフサイクルと公開切替ルールを追加。
  - `BD-API-001` と `BD-API-002` を更新し、利用者向け参照契約（静的）と管理画面向け更新契約（API）を分離。
  - `BD-UI-001` を更新し、管理画面更新系UIと利用者参照系UIの境界を明記。
  - `BD-DEP-004` を更新し、`/web/*` 配信領域と `/docs/*` 経路の公開境界を補強。
  - `DD-API-001` / `DD-API-005` を最小追従し、DB正本前提とタグ更新・公開反映APIを追加。
  - `AT-SCN-006` に静的JSON確認と情報漏えい確認観点を追加。
- 影響確認:
  - トレーサビリティ整合: `RQ-RDR-034 -> BD-ADR-021 -> BD-ARCH/DATA/API/DEP -> DD-API/AT-SCN` の経路を構築。
...
- 対象: `RQ-RDR-003`, `RQ-FR-005`, `RQ-FR-009`, `RQ-GL-005`, `RQ-DATA-001`, `RQ-INT-001`, `BD-ARCH-001`, `BD-ARCH-002`, `BD-ARCH-003`, `BD-ARCH-004`, `BD-ERD-001`, `BD-API-003`, `BD-UI-001`, `BD-UI-003`, `BD-DEP-004`, `BD-DATA-001`
- 実施:
  ...
  - `BD-ARCH-002/003/004` を全面更新し、論理境界・クラウド配置・主要データフローを3層構造へ再編。
  - `BD-ERD-001` を実体化し、収集実行/タグ管理/公開反映（publish run）をER図へ反映。
  - `BD-API-003` を実体化し、更新系/参照系分離のエラーモデルを定義。
  - `BD-UI-001` と `BD-UI-003` に配信反映ジョブ画面（UI-A06）と失敗復帰導線を追加。
  - `BD-DEP-004` に Phase A-D の段階実行計画を追加し、実行順序を明示。
- 影響確認:
  - アーキテクチャ整合: `BD-ARCH-001` をハブに、`ARCH-002/003/004` と `ERD/API/UI/DEP` の責務分割を統一。
  - データ整合: タグ/動画の正本管理、publish run、公開切替の責務をRQ-BD間で接続。
  - 運用整合: 管理画面拡張（反映実行・再試行・ロールバック確認）を設計文書と実行計画で一貫化。

## 追記（MVP確定APIの過不足見直しとDD処理ロジック明記）
- 対象: `DD-API-001`〜`DD-API-010`, `DD-API-011`, `DD-API-012`, `DD-API-013`, `DD-API-014`, `DD-API-015`, `RQ-RTM-002`, `BD-ADR-021`
- 実施:
  - MVP対象APIを `DD-API-001` に明示し、運用API正本経路を `/api/v1` へ統一。
  - 既存API（`DD-API-002/003/004/005/008/009/010`）へ処理ロジック、状態遷移、エラーマッピングを追記。
  - 予約境界API（`DD-API-006/007`）をMVP非対象として明記。
  - 不足APIとして `DD-API-011`（収集結果明細）、`DD-API-012`（配信前後再確認）、`DD-API-013`（タグ管理）、`DD-API-014`（ドキュメント公開実行）、`DD-API-015`（配信反映ジョブ状態）を新規追加。
  - `RQ-RTM-002` のDD-API行をFR単位へ再マッピングし、新規APIの追跡行を追加。
  - 設計追加に伴い `BD-ADR-021` を更新し、管理系API責務分割を記録。
- 影響確認:
  - 要求整合: `RQ-FR-005/017/018/019/024/025` の管理画面運用要件に対して、MVPで必要なAPIが欠落なく定義された。
  - 設計整合: `RQ-RDR-034 -> BD-ADR-021 -> DD-API-001/005/011..015` の追跡経路を構築。
...
- 影響確認:
  - テスト整合: DD-API-002/003/008/009 の経路仕様と IT/AT 手順が一致した。
  - 運用整合: 旧経路依存による手順誤実行リスクを低減した。

## 追記（TypeScript防御的型付けの設計・スキル反映）
- 対象: `BD-ADR-022`, `BD-BUILD-001`, `DD-CODE-001`, `.opencode/skills/doc-bd-build/SKILL.md`, `.opencode/skills/doc-bd-build/TEMPLATE.md`, `.opencode/skills/doc-dd-code/SKILL.md`, `.opencode/skills/doc-dd-code/TEMPLATE.md`
- 実施:
  - `BD-ADR-022` を新規作成し、「覚える運用」ではなく「型とツールで事故を塞ぐ」方針を設計決定として記録。
  - `BD-BUILD-001` を更新し、Brand/Opaque、境界decode、センチネル値禁止、`NonEmptyArray`、unsafe cast局所化、資源解放保証を品質ゲートへ追加。
  - `DD-CODE-001` を更新し、防御的型付け規約（裸ID回避、判別可能union、boolean blindness回避、shotgun parsing防止）を実装規約として具体化。
  - `doc-bd-build` と `doc-dd-code` のスキル/テンプレートを更新し、今後の文書改訂でも同観点が必須になるよう同期。
- 影響確認:
  - 設計整合: `BD-ADR-022 -> BD-BUILD-001 -> DD-CODE-001` の追跡経路を構築し、方針から実装規約まで一貫化。
...
- 対象: `RQ-RDR-035`, `RQ-OBY-001`, `BD-ADR-022`, `DD-LOG-001`, `DD-SEC-001`, `AT-OPS-001`
- 実施:
  ...
  - `BD-ADR-022` を新規作成し、ログ収集経路（stdout -> CloudWatch）、サンプリング方針、保持方針を設計決定として固定。
  - `DD-LOG-001` を更新し、共通JSONスキーマ、イベント語彙、ログレベル運用、セキュリティ制約、保持削除ルールを具体化。
  - `DD-SEC-001` の監査ログ保持期間を30日へ更新。
...
- 対象: `BD-UI-002`, `BD-UI-004`, `BD-SEC-001`, `BD-QUAL-001`, `RQ-RTM-001`, `RQ-RTM-002`
- 実施:
  - `BD-UI-002` に `[[RQ-UX-003]]` / `[[RQ-UX-013]]` / `[[RQ-UX-015]]` / `[[RQ-UX-020]]` の設計要点（入力再利用、ヘルプ導線、将来拡張時代替導線）を追加。
  - `BD-UI-004` に `[[RQ-UX-017]]` の状態通知要件（`aria-live`、通知文構成、再確認導線）を追加。
  - `BD-SEC-001` に `[[RQ-UX-022]]` の外部サービス評価（VPAT等）と代替導線・再評価運用を追加。
  - `BD-QUAL-001` に `[[RQ-UX-023]]` の受付SLA/是正プロセス品質を追加。
...
- 対象: `DD-ERR-001`, `DD-API-001`, `DD-COMP-001`, `DD-DDL-001`, `DD-AV-001`, `DD-PERF-001`, `DD-SCALE-001`
- 実施:
  - `DD-ERR-001` を Problem Details（`application/problem+json`）正本へ更新し、必須/拡張メンバー（`type/title/status/detail/instance` + `code/retryable/trace_id/errors[]` 等）を明記。
  - `DD-API-001` の共通エラーモデルを Problem Details 基準へ統一し、`traceId` 表記を `trace_id` に統一。
  - API/エラー契約の追跡性を補強するため、`DD-ERR-001` と `DD-API-001` に `BD-API-003` / `BD-API-005` 参照を追加。
  - アーキテクチャ/データ/品質の追跡不足補完として、`DD-COMP-001` に `BD-ARCH-002` / `BD-ARCH-003` / `BD-UI-003`、`DD-DDL-001` に `BD-ERD-001`、`DD-AV-001` / `DD-PERF-001` / `DD-SCALE-001` に `BD-QUAL-001`（`DD-SCALE-001` は `BD-ARCH-004` も）を追加。
- 影響確認:
  - 設計整合: `BD-API-003/005` の契約方針とDDエラー設計の矛盾（Problem Details未準拠）を解消。
  - トレーサビリティ整合: 優先不足対象（API共通方針/エラーモデル/論理構成/ERD/品質特性）からDDへの参照経路を補強。
  - 運用整合: 監視・ログ相関キーを `trace_id` に統一し、`DD-LOG-001` との参照整合を維持。

## 追記（インフラBD/DD文書の新規追加）
- 対象: `BD-ADR-026`, `BD-ARCH-006`, `BD-DEP-005`, `BD-MON-003`, `DD-DEP-003`, `DD-SEC-002`, `DD-AV-002`, `DD-LOG-002`
- 実施:
  - `BD-ADR-026` を新規追加し、インフラ設計を配信境界/運用境界/監視境界で分割する方針を決定記録化。
  - `BD-ARCH-006` を新規追加し、経路分離（`/web/*`, `/docs/*`, `/openapi/*`, `/api/v1/*`）を前提にしたインフラ境界アーキテクチャを定義。
  - `BD-DEP-005` を新規追加し、docs/front/backend/infra の領域分割デプロイ設計とロールバック方針を追加。
  - `BD-MON-003` を新規追加し、可用性/性能/セキュリティ/ログ品質の4軸SLI/SLO運用を定義。
  - `DD-DEP-003` を新規追加し、領域分割配備手順、経路別反映確認、切戻し条件を詳細化。
  - `DD-SEC-002` を新規追加し、経路認証境界、IAM最小権限、秘密情報統制を詳細化。
  - `DD-AV-002` を新規追加し、経路別可用性指標と一次復旧判定を詳細化。
  - `DD-LOG-002` を新規追加し、インフラ監視イベント語彙とログ品質ゲートを詳細化。
- 影響確認:
  - 設計整合: `BD-ADR-026 -> BD-ARCH-006/BD-DEP-005/BD-MON-003 -> DD-DEP-003/DD-SEC-002/DD-AV-002/DD-LOG-002` の追跡経路を構築。
...
- 対象: `BD-TST-001`, `BD-ADR-026`
- 実施:
  - `BD-TST-001` を更新し、単体テスト/結合テスト/受入テストの責務境界を明確化。
  - レベル間接続（UT完了 -> IT着手 -> AT着手）と、Fail時の進行停止ルールを品質ゲートとして追加。
  - 要求保証マトリクス（簡易）を追加し、要求カテゴリごとの主要検証層と最終保証層を明記。
  - 設計決定 `BD-ADR-026` を新規作成し、三層品質ゲート採用の理由と影響範囲を記録。
- 影響確認:
  - 設計整合: `BD-ADR-026 -> BD-TST-001 -> UT/IT/AT計画` の追跡経路を構築。
...
- 対象: `RQ-RDR-034`, `RQ-GL-002`, `RQ-FR-018`, `BD-ADR-004`, `BD-ADR-021`, `BD-ARCH-001`, `BD-ARCH-003`, `BD-ARCH-004`, `BD-API-001`, `BD-API-002`, `DD-API-001`, `DD-API-002`, `DD-API-012`, `DD-API-014`, `RQ-RTM-002`
- 実施:
  ...
  - `BD-ADR-004` を「静的配信+API起動バッチ（単一Backend API）構成」へ改題し、別worker非採用と手動/定期の同一API起動を明記。
  - `BD-ADR-021` を更新し、収集/再確認/公開反映のrun実行が同一Backend API内で完結する方針を追記。
  - `BD-ARCH-001/003/004` を更新し、Batch Runner in API と Hono Monolith Runtime を明記して論理/配置/フローを同期。
  - `BD-API-001/002` を更新し、収集後生成タイミングと運用契約に「Backend API内バッチ実行」を追記。
  - `DD-API-001/002/012/014` を更新し、キュー/ワーカー投入表現を「同一Backend API内ジョブ実行モジュールへの登録」へ統一。
  - `RQ-RTM-002` の `BD-ADR-004` 行を新タイトル・根拠RDRへ更新。
- 影響確認:
  - 要求整合: `RQ-RDR-034 -> RQ-GL-002/RQ-FR-018` で、バッチ定義と管理画面運用要求がモノリス前提に一致。
  - 設計整合: `RQ-RDR-034 -> BD-ADR-004/021 -> BD-ARCH/API -> DD-API` の経路で「単一Backend API内実行」を一貫追跡可能。
...
- 実施:
  ...
  - `AT-RPT-001` のAPI逆引きとAPI単位判定に [[DD-API-012]] / [[DD-API-013]] / [[DD-API-015]] を追加。
- 影響確認:
  - 受入整合: 要求（[[RQ-FR-019]]）と受入手順（[[AT-SCN-007]]）の間で、運用フローを図と手順の両方で追跡可能化。
  - 判定整合: JSON検証失敗時の未反映、公開反映時の状態監視、旧公開版維持の判断が受入記録項目へ反映された。
  - トレーサビリティ整合: `AT-PLAN -> AT-SCN-007 -> AT-RPT` の記録経路に `DD-API-012/013/015` の逆引きを接続。

## 追記（API/画面/バッチ/バッチイベント一覧の詳細設計リンク整備）
- 対象: `BD-UI-001`, `BD-ARCH-001`（`BD-API-001` は既存表で要件充足を確認）
- 実施:
  - `BD-UI-001` の画面カタログへ `詳細設計` 列を追加し、画面ごとのDD-UI参照先を明記。
  - `BD-ARCH-001` に `バッチ一覧` と `バッチイベント一覧` を新設し、run状態遷移とDD-API/DD-DDL/DD-LOG参照を追加。
  - `BD-API-001` は既存で API一覧表 + `詳細設計` 列 + 個別DDリンクが成立していることを確認し、追記対象から除外。
- 影響確認:
  - 設計整合: `BD-UI-001 -> DD-UI-*`、`BD-ARCH-001 -> DD-API/DD-DDL/DD-LOG` の参照経路を明確化し、`BD-API-001 -> DD-API-*` は既存整備済みであることを確認。
...
- 影響確認:
  ...
  - テスト整合: `BD-TST-001` のUT/IT/AT三層責務を、RTM上で機械的に監視できる。
  - 運用整合: `task docs:trace` 実行のみで設計別ビューとテスト層トレースを同時再生成できる。

## 追記（監視文書の機械的記述解消とADR参照整合）
- 対象: `BD-MON-001`, `BD-MON-002`, `BD-MON-003`
- 実施:
  - `BD-MON-001` を再構成し、監視対象/観測点/SLI定義/欠測時運用/[[RQ-GL-011|再収集]]判定条件を具体化。
  - `BD-MON-002` をSLO運用文書として再定義し、SLO目標・エラーバジェット・通知レベル・日次/月次判定・逸脱時フローを追加。
  - `BD-MON-003` の `related` と変更履歴のADR参照を、監視文脈に合わせて `[[BD-ADR-006]]` / `[[BD-ADR-014]]` / `[[BD-ADR-022]]` へ整理。
- 影響確認:
  - 設計整合: `BD-MON-001`（監視対象定義）と `BD-MON-002`（SLO運用判定）の責務分離が成立。
  - トレーサビリティ整合: `RQ-OBY-001` / `RQ-AV-001` / `RQ-PS-001` の受入閾値を監視文書上で直接追跡可能。
  - 運用整合: 欠測時保留、再収集条件、恒久対処への切替条件が文書化され、日次判定の再現性を確保。

## 追記（インフラ系文書のADR参照正規化）
- 対象: `BD-ARCH-006`, `BD-DEP-005`, `DD-AV-002`, `DD-DEP-003`, `DD-LOG-002`, `DD-SEC-002`, `RQ-RTM-002`
- 実施:
  - インフラ境界/配備経路/監視ログの根拠ADRを `[[BD-ADR-026]]` から文脈一致するADRへ再割当。
    - 経路境界・認証境界: `[[BD-ADR-014]]`
    - 監視SLO最小セット: `[[BD-ADR-006]]`
    - CloudWatch構造化ログ30日保持: `[[BD-ADR-022]]`
...
- 対象: `BD-DEP-005`
- 実施:
  - `BD-DEP-005` に `## AWSリソース一覧` を追加し、リソース名/個数/構築理由/根拠文書を表形式で整理。
...
- 影響確認:
  - 設計整合: `BD-DEP-005` 単体でインフラ構成の説明責務を満たし、関連文書（`BD-ARCH-006`, `BD-DEP-004`, `DD-SEC-002`, `DD-LOG-001`, `DD-COST-001`）へ追跡可能。
