...
- 対象: `RQ-UX-001`, `RQ-UX-005`, `RQ-RDR-032`, `BD-UI-002`, `DD-UI-002`, `BD-ADR-017`
- 実施:
  ...
  - 設計決定記録 `BD-ADR-017` を新規作成し、モバイルファースト採用理由と却下案を明文化。
- 影響確認:
  - 要求整合: `RQ-RDR-032 -> RQ-UX-001/RQ-UX-005` で端末優先順位の判断根拠と受入条件が追跡可能。
  - 設計整合: `BD-ADR-017 -> BD-UI-002 -> DD-UI-002` の経路で、設計意図と実装仕様の接続を確認。
...
- 対象: `BD-ADR-017`
- 実施:
  - `BD-ADR-017` を新規作成し、タグ原本のDB正本化と利用者向けJSON配信分離を設計判断として記録。
  - 更新イベント起点/定期バッチでの再生成を採用し、利用時点での都度再生成を不採用とする理由を明記。
- 影響確認:
  - 設計整合: `RQ-RDR-031 -> BD-ADR-017 -> BD-API-001/BD-DATA-001` の追跡経路を構築。
