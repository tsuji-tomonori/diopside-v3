...
- 実施:
  - `BD-UI-001` をテンプレート記述から再構成し、利用者導線（一覧/検索/詳細）と管理者導線（収集実行/監視/再収集/配信前後確認/公開運用）の画面カタログを追加。
...
- 影響確認:
  ...
  - 設計整合: `BD-ADR-009/010` で定義した補助表示方針（波形/ワードクラウド）を、画面遷移上の補助遷移として反映。
