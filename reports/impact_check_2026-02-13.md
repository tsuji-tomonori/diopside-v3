...
- 追加実施（2026-02-13）:
  - 要求決定記録/設計決定記録の追加: `RQ-RDR-038`, `BD-SYS-ADR-027`
  - 要求修正: `RQ-DG-001`, `RQ-FR-020`, `RQ-FR-022`, `RQ-FR-023`, `RQ-RTM-001`
  - 設計修正: `BD-SYS-ARCH-001`, `BD-APP-DATA-001`, `BD-APP-API-001`, `BD-APP-API-002`, `BD-APP-API-005`, `BD-INF-MON-001`, `DD-APP-API-002`, `DD-APP-API-014`, `DD-APP-LOG-001`
  - テスト/DDL補完: `AT-SCN-003`, `IT-CASE-009`, `IT-CASE-010`, `IT-CASE-011`, `DD-APP-DB-006`〜`DD-APP-DB-017`
  ...
  - AWSサービス一覧の明確化（2026-02-13 追補）:
    ...
    - 配備設計の一覧粒度をサービス単位へ統一: `BD-INF-DEP-005`
    - 設計決定記録へ一覧正本管理ルールを追記: `BD-SYS-ADR-028`
  ...
  - 追加実施（2026-02-13: a11y/UI/UX非機能設計レビュー対応）:
    ...
    - CIゲート具体化: `BD-DEV-PIPE-001`
    - 追跡更新: `RQ-RTM-001`
  - 追加実施（2026-02-13: API語彙統一・DB正本整合）:
    - API設計整合: `BD-APP-API-002` の実行入力を `trigger_mode` + `run_kind` へ分離し、状態語彙へ `cancelled` を追加
    - デプロイ整合: `BD-INF-DEP-006` をDB正本前提へ更新し、DBバックアップ（スナップショット/PITR）を追加
  ...
  - 追加実施（2026-02-13: テスト計画/判定追随・RTM直接リンク補強）:
    ...
    - 設計根拠要件補強: `DD-APP-MOD-002`, `DD-SYS-AV-002`, `DD-INF-IAC-002`, `DD-INF-MON-002`, `DD-INF-SEC-003`
  ...
  - 追加実施（2026-02-13: INF詳細の設定値・根拠整理）:
    - BD責務整理: `BD-INF-NET-001` を境界方針の正本へ再整理し、CIDR等の具体値正本を `DD-INF-NET-001` へ集約
    - VPC詳細化: `DD-INF-NET-001` を章分割（VPC/Subnet, Route, SG/NACL, VPC Endpoint）し、環境別CIDR具体値（`10.20/10.21/10.22`）を確定
    - IaC詳細化: `DD-INF-IAC-002`, `DD-INF-IAC-003` にリソース別設定責務とドリフト是正フロー（P1/P2/P3）を追加
    - DR/監視詳細化: `DD-INF-SEC-001`, `DD-INF-MON-001` にリソース別設定値・目標値・通知条件の根拠を追加
...
- テスト整合: DD-APP-API-011〜015に対応するUT、主要運用導線をカバーするIT/ATシナリオが追加され、`RQ -> BD/DD -> UT/IT/AT` の経路を補強。
...
- 追跡整合（追加）: `RQ-RTM-001` と `IT-CASE-009/010/011` の直接リンクを補強し、`DD-APP-API-011〜015` へのIT層トレースを明示した。
...
- 設計整合（AWS一覧追補）: `BD-INF-PLAT-001` と `BD-INF-DEP-005` の両方で、管理対象AWSサービスの個数・構築理由・導入段階を同一粒度で参照可能にした。
...
- 検証整合（a11y追補）: `AT-SCN-001/002` にキーボード・SR・320px/400%ズーム・言語指定・状態通知の受入観点を追加し、`RQ-UX-021` のCIゲート要件を `BD-DEV-PIPE-001` へ接続した。
...
- テスト・判定整合（2026-02-13追補）: UT/IT/AT計画・判定文書の対象範囲を `DD-APP-API-015` / `AT-SCN-009` まで拡張し、Go/No-Go入力の範囲不一致を解消した。
...
- `task docs:guard` 再実行で対象変更文書（`BD-APP-API-002`, `BD-INF-DEP-006`）の整合を確認（issues/broken links ともに 0）。
...
- `task docs:guard` 実行で、INF詳細更新6文書（`BD-INF-NET-001`, `DD-INF-NET-001`, `DD-INF-IAC-002`, `DD-INF-IAC-003`, `DD-INF-SEC-001`, `DD-INF-MON-001`）の `issues: 0` / `broken_links: 0` を確認。
