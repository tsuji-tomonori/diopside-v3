...
## 実施内容（INF-BD必須設計項目テンプレート改善）
- 対象: `docs/2.基本設計(BD)/04.インフラ(INF)` の18文書（PLAT/ENV/SEC/NET/EDGE/WAF/CMP_DEP/DATA/KMS/MON/SLO/AUD/DR/CICD/IAC/CM/OPS/QUAL）。
- 変更内容:
  - 全18文書へ `## DDへの引渡し（根拠/入力）` を追加し、DDへ渡す入力（境界、採否理由、判定条件）を明文化。
  - `## 必須設計項目` を `## 必須設計項目（BDで必ず決める）` へ統一し、BDで確定すべき判断項目を明確化。
  - 章構成の不足を補完し、`目的` / `未指定事項` / `受入基準` を追記または正規化（`受入条件` は対象文書で `受入基準` に統一）。
  - 各対象文書の frontmatter `updated` を `2026-02-23` に更新し、意味変更に伴う `version` をPATCH更新。

## 影響確認（INF-BD必須設計項目テンプレート改善）
- BDで確定する判断（境界/採否理由/合否）と、DDで確定する実装値（CIDR/IAM詳細/閾値/保持日数/手順）が文書上で分離され、責務境界の追跡性が向上した。
- 章ごとのDD入力が明文化されたことで、INF配下 `DD-INF-*` への引き渡し漏れを抑止できる構成になった。
- 既存ID・配置・リンクは維持し、RTM/既存参照への互換性を保ったまま改善した。
