...
- `UT-PLAN-005` は対象を `DD-APP-API-002..009` と `UT-CASE-BE-001..008` に固定（`docs/4.単体テスト(UT)/11.単体テスト計画(PLAN)/UT-PLAN-005.md:40`, `docs/4.単体テスト(UT)/11.単体テスト計画(PLAN)/UT-PLAN-005.md:52`）。
- `IT-PLAN-001` も同様に `DD-APP-API-002..009` / `IT-CASE-001..008` を前提（`docs/5.結合テスト(IT)/11.結合テスト計画(PLAN)/IT-PLAN-001.md:24`, `docs/5.結合テスト(IT)/11.結合テスト計画(PLAN)/IT-PLAN-001.md:42`）。
- 一方、RTM設計別ビューは `DD-APP-API-010..015`、`UT-CASE-BE-009..013`、`IT-CASE-009..013` まで追跡。

### High-2: 受入計画/判定文書が最新受入シナリオを網羅していない
- `AT-PLAN-001` の受入シナリオ一覧に `AT-SCN-006/008/009` がない（`docs/6.受入テスト(AT)/11.受入テスト計画(PLAN)/AT-PLAN-001.md:38`）。
- `AT-GO-001` の Go 条件は `AT-SCN-001..005` と `DD-APP-API-002..009` に限定（`docs/6.受入テスト(AT)/52.リリース判定(GO)/AT-GO-001.md:40`, `docs/6.受入テスト(AT)/52.リリース判定(GO)/AT-GO-001.md:41`）。
- `AT-RPT-001` も関連/テンプレートが同様に未網羅（`docs/6.受入テスト(AT)/61.受入テスト報告(RPT)/AT-RPT-001.md:17`, `docs/6.受入テスト(AT)/61.受入テスト報告(RPT)/AT-RPT-001.md:73`）。
...
- 例: `DD-APP-MOD-002`, `DD-SYS-AV-002`, `DD-INF-IAC-002`, `DD-INF-MON-002`, `DD-INF-SEC-003` は設計別ビューで根拠要件列が `-`。
...
## 4. 改善提案（実行順）
1. `UT-PLAN-005` と `IT-PLAN-001` を最新API範囲（`DD-APP-API-010..015`）に更新し、ケース一覧と完了条件の件数を実態に合わせる。
2. `AT-PLAN-001` / `AT-GO-001` / `AT-RPT-001` を `AT-SCN-001..009` と `DD-APP-API-002..015` の体系へ更新する。
...
- コーディング規約（`DD-DEV-CODE-001`）とレビュー規約（`DD-DEV-REV-001`）は妥当。実運用上は「承認ステータス運用」との連携強化が必要。
