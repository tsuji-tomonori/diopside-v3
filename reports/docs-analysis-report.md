...
| 詳細設計 (DD) | 60% | **82%** | 改善 | DD-APP-MOD-002追加、DDLカラム課題残存 |
| テスト設計 (UT/IT/AT) | 44% | **75%** | 改善 | 15ケース追加、RTM構造的乖離を新規検出 |
| 運用設計 | 65% | **88%** | 改善 | バックアップ/障害判定補完、CORS等3件追加 |
| 規約・標準 | 80% | **78%** | 微減 | DD-APP-MOD-002追加も、全文書下書き問題を新規検出 |
...
| Phase | ドキュメント | 内容 |
|:-----:|-------------|------|
| UT | UT-CASE-BE-009〜013 | DD-APP-API-011〜015対応の単体テストケース |
| IT | IT-CASE-009〜013 | E2E統合テスト・障害復旧シナリオ |
| AT | AT-SCN-008/009 | メタデータ正規化・タグ管理シナリオ |
| BD | BD-INF-DEP-006 | バックアップ・リカバリ設計 |
| DD | DD-APP-MOD-002 | Next.js App Router実装ガイド |
...
| BD-SYS-ARCH-001 | BAT-006/BAT-007とバッチ実行制約を追加 |
| DD-SYS-AV-001 | MTTR単一基準化、外部障害判定アルゴリズムを追加 |
| DD-SYS-COST-001 | コスト超過検知アルゴリズムを追加 |
...
- **推奨対応**: BD-APP-DATA-001にタグ辞書同期フローと整合性保証を明記
...
- **ステータス**: 未解決
- **問題**: RQ-PS-001でLCP p95 ≤2.5sを要求しているが、BD-SYS-ADR-005のクライアントサイド検索はデータ量増加でLCPが悪化する構造的リスクがある
- **参照**: `RQ-PS-001`, `BD-SYS-ADR-005`
- **影響**: 動画数が数千件を超えた場合、JSONダウンロード+クライアント検索がLCP目標を超過する
- **推奨対応**: BD-SYS-ADR-005にスケーラビリティ上限（対応可能な動画件数）を明記し、閾値超過時の移行計画を追加

#### [Medium] LLMタグ提案のエラーハンドリング未定義
- **ステータス**: 未解決
- **問題**: RQ-FR-019とBD-APP-API-002でLLMタグ提案機能が定義されているが、LLM APIのエラー時（タイムアウト、レート制限、不正応答）の挙動が未定義
- **参照**: `RQ-FR-019`, `BD-APP-API-002`
- **影響**: LLM API障害時に管理画面がハングアップまたは不正なタグが提案される
- **推奨対応**: BD-APP-API-002にLLMフォールバック戦略（手動タグ付けへの縮退）を追加

#### [Low] 単一Lambda + 99.5%稼働率のコールドスタート影響
- **ステータス**: 未解決
- **問題**: RQ-AV-001の99.5%稼働率要件に対し、BD-SYS-ARCH-001の単一Lambda構成ではコールドスタートが可用性に影響する
- **参照**: `RQ-AV-001`, `BD-SYS-ARCH-001`
...
- **推奨対応**: DD-SYS-COST-001のコスト内訳に余裕度（バッファ率）を明記
...
#### [High] 補助機能の生成責務が不明確
- **ステータス**: 解決済
- **問題**: 波形/ワードクラウド生成のバッチ処理がBD-SYS-ARCH-001のバッチ一覧に欠落
- **参照**: `BD-APP-API-001:70`, `BD-SYS-ARCH-001:61-69`
- **影響**: 生成バッチの運用設計・監視設計が漏れる
- **推奨対応**: BD-SYS-ARCH-001のバッチ一覧を拡張
- **解決内容**: BD-SYS-ARCH-001にBAT-006（波形/ワードクラウド生成）、BAT-007（タグマスター更新）を追加済み

#### [Medium] ADR間の依存関係が片方向のみ
- **ステータス**: 未解決
- **問題**: BD-SYS-ADR-021からBD-SYS-ARCH-001への参照はあるが、逆方向の参照がない
- **参照**: `BD-SYS-ADR-021:52-60`, `BD-SYS-ARCH-001:10-26`
- **影響**: ADR変更時の影響範囲が片方向からしか追跡できず、設計変更の見落としが生じる
- **推奨対応**: BD-SYS-ARCH-001のfrontmatterに`blockedBy: ['BD-SYS-ADR-001', 'BD-SYS-ADR-021', 'BD-SYS-ADR-024']`を追加
...
#### [High] バッチ処理設計が不完全
- **ステータス**: 解決済
- **問題**: 波形、ワードクラウド、タグマスター更新のバッチがBD-SYS-ARCH-001に未記載
- **参照**: `BD-SYS-ARCH-001:61-79`
- **影響**: バッチ運用の設計漏れにより、生成機能が未実装のまま残る
- **推奨対応**: BAT-006/BAT-007を追加
- **解決内容**: BD-SYS-ARCH-001にBAT-006/BAT-007と実行制約（最大実行時間、Retry、同時実行数）を追加済み

#### [Medium] API参照系vs更新系の正本が不明確
- **ステータス**: 未解決
- **問題**: BD-APP-API-001（API一覧）とBD-APP-API-002（収集API設計）で同じAPIの定義が異なるレベルで記載されており、正本が不明確
- **参照**: `BD-APP-API-001:52-65`, `BD-APP-API-002:45-66`
- **影響**: API仕様の矛盾が実装時に混乱を引き起こす
- **推奨対応**: BD-APP-API-001を正本一覧とし、BD-APP-API-002を詳細仕様として役割を明記

#### [Medium] LLMタグ取込→再生成の自動トリガー有無が不明
- **ステータス**: 未解決
- **問題**: LLMタグ提案を取り込んだ後、tag_masterの再生成が自動的にトリガーされるのか手動なのかが不明
- **参照**: `BD-APP-DATA-001:58`, `BD-APP-API-002:86-91`
- **影響**: タグ取込後の公開サイトへの反映タイミングが不確定になる
- **推奨対応**: タグ提案取込契約に「反映後処理」セクションを追加し、再生成トリガーを明記

#### [Medium] インフラのHA戦略が不足
- **ステータス**: 未解決
- **問題**: 冗長化、フェイルオーバーの具体的な設計がBD-INF-DEP-001/BD-INF-DEP-004に不足
- **参照**: `BD-INF-DEP-001`, `BD-INF-DEP-004`
- **影響**: 単一AZ障害時のサービス継続が保証されない
- **推奨対応**: 可用性設計として、S3のクロスリージョンレプリケーション検討、CloudFrontのオリジンフェイルオーバー設定を追加

#### [Medium] クライアントサイド検索のスケーラビリティ限界未定義
- **ステータス**: 未解決
- **問題**: BD-SYS-ADR-005でクライアントサイド検索を採用しているが、対応可能な動画件数の上限が未定義
- **参照**: `BD-SYS-ADR-005`
- **影響**: 動画数の増加に伴いフロントエンドの検索性能が劣化し、性能要件を満たせなくなる
- **推奨対応**: BD-SYS-ADR-005に「500件以下: クライアント検索、500件超: サーバー検索移行」等の閾値と移行トリガーを追加
...
- **ステータス**: 未解決
- **問題**: DD-APP-DB-002にDDL一覧があり、DD-APP-DB-005〜014に各テーブル定義が存在するが、一部カラムの型・制約・デフォルト値の詳細が不足している
- **参照**: `DD-APP-DB-006〜DD-APP-DB-017`
...
#### [High] DD-APP-MOD-002が存在しない
- **ステータス**: 解決済
- **問題**: Next.js App Router (RSC/Client境界) の実装ガイドが欠落していた
- **参照**: BD-SYS-ADR-024
- **影響**: フロントエンド実装者がRSC/Client境界の判断を個人裁量で行い、バンドルサイズ増大やハイドレーションエラーが発生する
- **推奨対応**: DD-APP-MOD-002として新規作成
- **解決内容**: DD-APP-MOD-002（Next.js App Router実装ガイド）を新規作成済み

#### [Medium] ESLint/tsconfig.json設定が未記載
- **ステータス**: 未解決
- **問題**: DD-DEV-CODE-001の規約を強制する実装設定（ESLintルール、tsconfig strict設定）が未記載
- **参照**: `DD-DEV-CODE-001`
...
#### [Medium] 冪等性の実装方法が未指定
- **ステータス**: 未解決
- **問題**: DD-APP-API-002で24時間有効期限の冪等性キーが定義されているが、保存先（インメモリ/DB/キャッシュ）が不明
- **参照**: `DD-APP-API-002:70-72`
...
- **ステータス**: 未解決
- **問題**: DD-APP-UI-002のコンポーネント間通信でイベントペイロード形式が未定義
- **参照**: `DD-APP-UI-002:37-42`
- **影響**: コンポーネント間の型安全性が保証されず、ランタイムエラーが発生する
- **推奨対応**: イベントペイロードのTypeScript型定義を追加

#### [Medium] State管理ライブラリ未選定
- **ステータス**: 未解決
- **問題**: DD-APP-UI-001でState管理の方針は述べられているが、具体的なライブラリが未選定
- **参照**: `DD-APP-UI-001:40-43`
...
- **ステータス**: 未解決
- **問題**: BD-APP-API-003でエラーモデルは定義されているが、400/422/207の判定基準表がない
- **参照**: `BD-APP-API-003:34-58`
- **影響**: 実装者によってHTTPステータスコードの使い分けが異なる
- **推奨対応**: 判定基準表（入力不正→400、バリデーション→422、部分成功→207）を追加

#### [Medium] エラーコードのAPI別割り当て表がない
- **ステータス**: 未解決
- **問題**: DD-APP-ERR-001に15パターンのエラーコードが定義されているが、各APIでどのコードを返すかの対応表がない
- **参照**: `DD-APP-ERR-001:42-44`
...
- **参照**: `BD-APP-API-002`, `DD-APP-API-002`
- **影響**: クォータ枯渇時に収集バッチが無限リトライするか、データ欠損が発生する
- **推奨対応**: DD-APP-API-002にクォータ監視とグレースフルデグラデーション（残クォータ確認→中断→翌日再開）を追加

#### [Medium] 同時バッチ実行の上限指定なし
- **ステータス**: 未解決
- **問題**: 複数の収集バッチが同時に起動された場合の制御が未定義（Lambda同時実行数との関係）
- **参照**: `BD-SYS-ARCH-001`, `DD-APP-API-002`
- **影響**: 同時実行によるDB負荷増大、YouTube APIクォータの急速消費
- **推奨対応**: BD-SYS-ARCH-001に同時実行制御（SQS FIFO or DynamoDBロック）を追加

#### [Medium] 失敗したpublish操作のロールバック手順未定義
- **ステータス**: 未解決
- **問題**: DD-APP-API-014（ドキュメント公開実行API）でpublish操作が部分的に失敗した場合（S3アップロード済み・CloudFront invalidation未完了等）のロールバック手順がない
- **参照**: `DD-APP-API-014`, `DD-APP-DB-015`, `DD-APP-DB-016`
...
- **参照**: `DD-INF-DEP-001:30-34`
- **影響**: 環境差異によるデプロイ失敗
- **推奨対応**: コンテキスト管理方法（Git管理 or パラメータストア）を明記

#### [Low] スキーマバージョン間のデータ移行手順の詳細不足
- **ステータス**: 未解決
- **問題**: DD-APP-DB-003にマイグレーション方針はあるが、具体的な移行スクリプト仕様が不足
- **参照**: `DD-APP-DB-003`
...
- **ステータス**: 未解決
- **問題**: DD-APP-UI-006に管理画面の概要はあるが、各画面（収集状態、配信管理等）の詳細レイアウト・操作フローが不足
- **参照**: `DD-APP-UI-006`
...
- **ステータス**: 未解決
- **問題**: DD-INF-DEP-001にデプロイ方式はあるが、CDKのスタック分割やコンストラクト命名規則が未記載
- **参照**: `DD-INF-DEP-001`
- **影響**: CDK実装時にスタック構成の議論が発生し、リファクタリングコストが増加
- **推奨対応**: CDKスタック構成図と命名規則をDD-INF-DEP-001に追加
...
- **参照**: `DD-APP-UI-002`, `DD-APP-ERR-001`
...
- **参照**: `DD-APP-API-003`, `DD-APP-UI-006`
...
|---------|:---:|:---:|------|
| 単体テスト(UT) | 53% (8/15 API) | 87% (13/15 API) | UT-CASE-BE-009〜013追加 |
| 結合テスト(IT) | 62% (8/13) | 100% (13/13) | IT-CASE-009〜013追加 |
| 受入テスト(AT) | 56% (14/25 FR) | 80% (20/25 FR) | AT-SCN-008/009追加 |

#### [High] UTカバレッジ漏れ
- **ステータス**: 解決済
- **問題**: DD-APP-API-011/012/013/014/015に対応するUTケースが存在しなかった
- **参照**: `DD-APP-API-011〜015`
- **影響**: 5つのAPIの品質が単体テストレベルで保証されない
- **解決内容**: UT-CASE-BE-009〜013を新規作成し、全15 APIの単体テストカバレッジを確保
...
#### [Medium] DD-APP-API-011〜015のIT層カバレッジ
- **ステータス**: 未解決
- **問題**: DD-APP-API-011〜015はUT-CASE（単体テスト）と一部AT-SCN（受入テスト）のマッピングがあるが、RTM-002の設計別ビューでIT-CASE（結合テスト）への直接マッピングがない
- **参照**: `RQ-RTM-002:131-135`
- **影響**: 収集結果明細API、配信前後再確認API等の結合テストカバレッジが間接パス依存
- **推奨対応**: IT-CASE-009〜013のfrontmatterに対応するDD-APP-API-011〜015への`up`リンクを追加
...
- **参照**: `BD-INF-DEP-001〜005`
- **影響**: データ消失時にリカバリ不可能
- **解決内容**: BD-INF-DEP-006（バックアップ・リカバリ設計）を新規作成。S3バージョニング、IaCによる構成復元、RTO/RPOを定義

#### [Critical] 可用性計算の障害分類が再現不可能
- **ステータス**: 解決済
- **問題**: 「15分以上断続失敗」基準が離散的で、月次集計時に再現不能だった
- **参照**: `DD-SYS-AV-001:47-52`
- **影響**: 可用性SLOの判定が恣意的になる
- **解決内容**: DD-SYS-AV-001に外部要因判定アルゴリズム（フローチャート形式）を追加し、月次集計の自動化手順を明記
...
- **解決内容**: DD-SYS-COST-001にコスト超過検知閾値と抑制施策を追加。ただしCost Anomaly Detection統合は未実装
...
- **参照**: `RQ-AV-001:36`
- **影響**: MTTR計算が再現不能
- **解決内容**: DD-SYS-AV-001でMTTR判定を単一基準化（「障害検知から一次復旧まで30分以内」に統一）

#### [Medium] CORSポリシーの明示的仕様がない
- **ステータス**: 未解決
- **問題**: API Gateway / CloudFrontのCORSポリシー（許可オリジン、メソッド、ヘッダー）が設計文書に未記載
- **参照**: `BD-APP-API-005`, `DD-SYS-SEC-001`
- **影響**: フロントエンドからのAPI呼び出しがCORSエラーでブロックされる、またはセキュリティ上過度に緩いポリシーが設定される
- **推奨対応**: BD-APP-API-005またはDD-SYS-SEC-001にCORSポリシー仕様（`Access-Control-Allow-Origin: https://diopside.example.com`等）を追加
...
- **推奨対応**: DD-SYS-SEC-001にCookie/セッション管理仕様を追加

#### [Low] CI/CDパイプラインの具体的構成仕様がない
- **ステータス**: 未解決
- **問題**: BD-DEV-PIPE-001で「CIチェック必須」と記載されているが、パイプラインのステージ構成（lint→test→build→deploy）の詳細がない
- **参照**: `BD-DEV-PIPE-001`
- **影響**: CI/CD構築時にステージ順序やゲート条件が実装依存になる
- **推奨対応**: BD-DEV-PIPE-001にパイプラインステージ定義（GitHub Actions workflow構成）を追加
...
- **解決内容**: DD-SYS-COST-001に検知閾値・抑制施策を追加済みだが、日次通知の具体的実装は未完

#### [Medium] 可観測性証跡の30日制限
- **ステータス**: 未解決
- **問題**: CloudWatch Logsの保持期間が30日に設定されており、31日超過のログは自動削除される
- **参照**: `DD-APP-LOG-001:114-116`
...
- **参照**: `RQ-AV-001:36`
- **影響**: MTTR SLO達成度が検証不能
- **解決内容**: DD-SYS-AV-001で単一基準化（障害検知から一次復旧まで30分以内）に統一
...
| 規約 | ドキュメント | 網羅性 |
|------|:----------:|:---:|
| コーディング規約 | DD-DEV-CODE-001 | 80% |
| レビュー規約 | DD-DEV-REV-001 | 90% |
...
- **ステータス**: 未解決
- **問題**: DD-DEV-CODE-001の規約を実行時に強制する設定ファイルの仕様が欠如
- **参照**: `DD-DEV-CODE-001`
...
- **解決内容**: DD-APP-MOD-002（Next.js App Router実装ガイド）を新規作成

#### [Medium] traceIdの取得・伝播方法
- **ステータス**: 未解決
- **問題**: DD-DEV-CODE-001で「非同期処理はtry/catch + traceId」と規定しているが、traceIdの生成元と伝播方法（リクエストヘッダー? Lambda context?）が未定義
- **参照**: `DD-DEV-CODE-001`, `DD-APP-LOG-001`
- **影響**: トレーシングが断片的になり、障害調査時にリクエストの追跡が困難
- **推奨対応**: DD-APP-LOG-001にtraceId生成・伝播パターン（X-Ray or カスタムヘッダー）を追加

#### [Medium] i18n仕様の欠如
- **ステータス**: 未解決
- **問題**: RQ-UX-024で「言語指定（Language of Page/Parts）」が要求されているが、実装仕様（`<html lang="ja">`以外のi18n対応範囲、多言語切替の有無）が未定義
- **参照**: `RQ-UX-024`, `BD-APP-UI-002`
- **影響**: WCAG 2.2 AAの言語指定要件への対応範囲が不明確
- **推奨対応**: BD-APP-UI-002にi18n方針（「日本語単一言語、`lang="ja"`固定」等）を明記

#### [Medium] APIレート制限仕様がない
- **ステータス**: 未解決
- **問題**: diopside APIのレート制限仕様が未定義（<10同時ユーザー想定のみ）
- **参照**: `BD-APP-API-005`, `DD-APP-API-001`
- **影響**: 管理APIへの過剰リクエストによるLambdaコスト増大やサービス不安定化
- **推奨対応**: BD-APP-API-005にAPIレート制限（管理API: 10req/min、公開API: 100req/min等）を追加

#### [Low] Result型の定義
- **ステータス**: 未解決
- **問題**: DD-DEV-CODE-001で推奨されるResult型パターンの具体的な型定義がない
- **参照**: `DD-DEV-CODE-001`
...
- **ステータス**: 未解決
- **問題**: DD-DEV-CODE-001で推奨されるBrand/Opaque型の配置場所（共通型定義ディレクトリ等）が不明
- **参照**: `DD-DEV-CODE-001`
...
| 3 | DDLカラム詳細の補完 | High | DD-APP-DB-006〜014 修正 | 未着手 |
| 4 | RTM自動生成の実装状態確認 | High | DD-API実装確認 | 未着手 |
| 5 | RDR同時更新の強制プロセス追加 | High | RQ-DG-001 修正 | 未着手 |
| ~~6~~ | ~~テストケース追加~~ | ~~High~~ | ~~UT/IT/AT~~ | **完了** |
| ~~7~~ | ~~バックアップ・リカバリ設計~~ | ~~Critical~~ | ~~BD-INF-DEP-006~~ | **完了** |
| ~~8~~ | ~~外部障害判定アルゴリズムの明確化~~ | ~~Critical~~ | ~~DD-SYS-AV-001~~ | **完了** |
...
| 2 | YouTube APIレート制限対応 | Medium | DD-APP-API-002 修正 |
| 3 | 同時バッチ実行制御 | Medium | BD-SYS-ARCH-001 修正 |
| 4 | publishロールバック手順 | Medium | DD-APP-API-014 修正 |
| 5 | CORSポリシー仕様 | Medium | BD-APP-API-005 修正 |
| 6 | APIレート制限仕様 | Medium | BD-APP-API-005 修正 |
| 7 | i18n方針の明記 | Medium | BD-APP-UI-002 修正 |
| 8 | traceId伝播方法 | Medium | DD-APP-LOG-001 修正 |
| 9 | クライアント検索スケーラビリティ上限 | Medium | BD-SYS-ADR-005 修正 |
| 10 | API参照系/更新系の正本明記 | Medium | BD-APP-API-001/002 修正 |
| 11 | DD-APP-API-011〜015のIT層直接マッピング | Medium | IT-CASE-009〜013 修正 |
...
| # | 対応項目 | 重大度 | 対象ドキュメント |
|:---:|----------|:---:|----------------|
| 1 | Cookie/セッション管理仕様 | Low | DD-SYS-SEC-001 修正 |
| 2 | CI/CDパイプライン構成仕様 | Low | BD-DEV-PIPE-001 修正 |
| 3 | CDKスタック構成・命名規則 | Low | DD-INF-DEP-001 修正 |
| 4 | 管理UI画面の詳細設計 | Low | DD-APP-UI-006 拡張 |
| 5 | スキーマ移行手順の詳細化 | Low | DD-APP-DB-003 修正 |
| 6 | Result型/Brand型の標準定義 | Low | DD-DEV-CODE-001 修正 |
| 7 | debounce/throttle標準パラメータ | Low | DD-APP-UI-002 修正 |
...
| Doc-ID | タイトル |
|--------|---------|
| BD-SYS-ADR-001〜026 | アーキテクチャ決定記録（26件） |
| BD-SYS-ARCH-001〜006 | システムアーキテクチャ（コンテキスト、論理構成、配置、データフロー、AI運用、インフラ境界） |
| BD-APP-DATA-001 | データアーキテクチャ |
| BD-APP-DATA-002 | ER図（概要） |
| BD-APP-DATA-003 | データ保持・削除方針 |
| BD-APP-API-001〜005 | API設計（一覧、収集API、エラーモデル、OpenAPI、契約方針） |
| BD-APP-UI-001〜004 | UI設計（画面一覧、検索画面、画面遷移、アクセシビリティ） |
| BD-DEV-ENV-001〜002 | 環境設計（開発、本番） |
| BD-INF-CM-001 | 構成管理方針 |
| BD-DEV-PIPE-001 | ビルド方針 |
| BD-INF-MON-001〜003 | 監視設計（監視、SLO運用、インフラ監視） |
| BD-INF-DEP-001〜006 | デプロイ設計（方式、運用ルール、Quartz公開、CloudFront、インフラ、バックアップ） |
| BD-SYS-SEC-001 | セキュリティ設計 |
| BD-SYS-QUAL-001 | 品質特性 |
...
| Doc-ID | タイトル |
|--------|---------|
| DD-APP-MOD-002 | Next.js App Router実装ガイド |
| DD-APP-MOD-001〜002 | コンポーネント分割、責務対応表 |
| DD-APP-DB-002〜014 | DDL定義（一覧 + 13テーブル） |
| DD-APP-DB-001〜002 | DB制約（方針、一意制約・チェック制約） |
| DD-APP-DB-003 | DB移行方針 |
| DD-APP-API-001〜015 | API詳細設計（総論 + 14 API） |
| DD-APP-UI-001〜006 | UI詳細（総論、一覧、フィルタ、詳細、検索、運用ステータス） |
| DD-APP-ALG-001 | 検索アルゴリズム |
| DD-APP-LOG-001〜002 | ログ設計（アプリ、インフラ） |
| DD-APP-ERR-001 | エラーコード設計 |
| DD-SYS-PERF-001 | 性能設計 |
| DD-SYS-AV-001〜002 | 可用性詳細（アプリ、インフラ） |
| DD-SYS-SEC-001〜002 | セキュリティ統制（アプリ、インフラ） |
| DD-INF-DEP-001〜003 | デプロイ詳細（方式、CloudFront、インフラ領域分割） |
| DD-SYS-SCALE-001 | 拡張性詳細 |
| DD-SYS-COST-001 | コスト運用詳細 |
| DD-DEV-CODE-001 | コーディング規約 |
| DD-DEV-REV-001 | レビュー規約 |
...
| UT-CASE-BE-001〜013 | テストケース（API単体テスト 13件） |
| UT-MOCK-001 | モック方針 |
